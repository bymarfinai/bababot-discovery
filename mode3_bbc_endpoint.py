"""Mode3 BBC Backtest Endpoint — v2.5: 4H directional filter."""
import os
...
def compute_higher_tf_ema(rows_1h, rows_4h, ema_period):
    """Compute 4H EMA and map to 1H bar indices."""
    if not rows_4h: return None
    closes_4h = np.array([r[4] for r in rows_4h], dtype=float)
    ema_4h = compute_ema_series(closes_4h, ema_period)
    # Map 4H timestamps to EMA values
    ts_to_ema = {rows_4h[i][0]: float(ema_4h[i]) for i in range(len(rows_4h))}
    # For each 1H bar, find the most recent completed 4H bar's EMA
    result = []
    sorted_4h_ts = sorted(ts_to_ema.keys())
    for r in rows_1h:
        ts_1h = r[0]
        # Find the 4H bar that contains this 1H bar (floor to 4H boundary)
        # 4H bars are at 0:00, 4:00, 8:00, 12:00, 16:00, 20:00
        ms_4h = 4 * 3600 * 1000
        aligned_ts = (ts_1h // ms_4h) * ms_4h
        ema_val = ts_to_ema.get(aligned_ts)
        if ema_val is None:
            # Try previous 4H bar
            prev_ts = aligned_ts - ms_4h
            ema_val = ts_to_ema.get(prev_ts)
...
def backtest_mode3_bbc(
    ...
    higher_tf_filter_enabled:bool=Query(False),
    higher_tf_ema_period:int=Query(7,ge=3,le=50),
    ...
    config = Mode3BBCConfig(
        ...
        higher_tf_filter_enabled=higher_tf_filter_enabled, higher_tf_ema_period=higher_tf_ema_period,
    ...
    # v2.5: compute 4H EMA and map to 1H bars
    if config.higher_tf_filter_enabled and timeframe == '1h':
        rows_4h = load_candles_from_db(symbol, '4h', start_ts, end_ts)
        if rows_4h:
            htf_ema = compute_higher_tf_ema(rows, rows_4h, config.higher_tf_ema_period)
            switcher.higher_tf_ema_series = htf_ema
    ...
    return {"symbol":symbol,"timeframe":timeframe,"days":days,"end_days_ago":end_days_ago,
        ...
        "summary":{"total_trades":n,"win_rate_pct":round(wr,2),"wins":len(wins),"losses":len(losses),
            ...
            "bull_blocked_higher_tf":switcher._bull_blocked_higher_tf,
            "bear_blocked_higher_tf":switcher._bear_blocked_higher_tf,
...
def mode3_bbc_health():
    return {"status":"ok","module":"mode3_bbc","version":"2.5-4h-filter","db_path":DB_PATH}
