"""BTC Temporal Saturday18 A7.0 — independent money geometry discovery.

Entry: every Saturday exact 18:00 WIB 5m open.
Direction: BUY only.
Evaluation: same 971-day window and execution assumptions as Tuesday/Friday temporal research.
Research only; does not mutate live BBC or frozen Tuesday champion.
"""
import json
...
def trade(rows,i,tp,sl,hold):
    ...
    gross_pct=100.0*(exit_px-e)/e; net_pct=gross_pct-FEE_PCT
...
def subset_summary(ts):
    ...
    return {'n':n,'wr':rnd(100*sum(x>0 for x in pnls)/n,2),'pnl':rnd(sum(pnls),3),
            'exp':rnd(sum(pnls)/n,4),'pf':rnd(pos/neg,3) if neg>0 else None,
            'mdd':rnd(max_dd(pnls),3),'ls':loss_streak(pnls)}


def summarize(ts,tp,sl,hold):
    pnls=[x['net_usd'] for x in ts]; n=len(ts); pos=sum(x for x in pnls if x>0); neg=-sum(x for x in pnls if x<0); wins=sum(x>0 for x in pnls)
    blocks=[]
    for b in range(8):
    ...
    return {'tp_pct':tp,'sl_pct':sl,'hold_min':hold,'rr':rnd(tp/sl,3),'trades':n,
            ...
            'net_pnl_usd':rnd(sum(pnls),3),'expectancy_usd':rnd(sum(pnls)/n,4),'profit_factor':rnd(pos/neg,3) if neg>0 else None,
            'max_dd_usd':rnd(max_dd(pnls),3),'max_loss_streak':loss_streak(pnls),
            'positive_blocks':sum(x>0 for x in blocks),'block_net_usd':blocks,'avg_hold_min':rnd(sum(x['bars']*5 for x in ts)/n,2)}


def raw_stats(rows,idx,h):
    vals=[]
    k=h//5
    for i in idx:
        j=i+k
        if j>=len(rows) or rows[j][0]!=rows[i][0]+k*TF: continue
        vals.append(100.0*(rows[j][1]-rows[i][1])/rows[i][1])
    n=len(vals)
    return {'n':n,'wr':rnd(100*sum(x>0 for x in vals)/n,2),'avg':rnd(sum(vals)/n,4),'median':rnd(sorted(vals)[n//2] if n%2 else (sorted(vals)[n//2-1]+sorted(vals)[n//2])/2,4)}
...
def main():
    ...
    raw={'full':{},'discovery':{},'validation':{}}
    for h in RAW_H:
        raw['full'][str(h)]=raw_stats(rows,idx,h)
        raw['discovery'][str(h)]=raw_stats(rows,idx[:split],h)
        raw['validation'][str(h)]=raw_stats(rows,idx[split:],h)
    results=[]
    for hold in HOLDS:
      for tp in TPS:
       for sl in SLS:
    ...
    out={'status':'SATURDAY18_A70_MONEY_GEOMETRY','data':{'coverage_pct':rnd(100*exact/expected,2),'rows_5m':exact,'saturdays':len(idx),'discovery_n':split,'validation_n':len(idx)-split,'entry':'Saturday 18:00 WIB exact open','direction':'BUY','fee_roundtrip_pct':FEE_PCT,'notional_usd':NOTIONAL,'configs':len(results)},'raw_directional':raw,'best_net':bynet[:20],'best_cross_period':cross[:20],'best_stable':stable[:20],'best_balanced':balanced[:20],'highest_wr_profitable':highwr[:20]}
    print('COVERAGE',exact,expected,rnd(100*exact/expected,2),'SATURDAYS',len(idx),flush=True)
