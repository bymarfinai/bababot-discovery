...
- dari kasus tersebut, berapa yang akhirnya tetap mencapai E20 setelah retest selesai dikonfirmasi;
...
3. karena `back_in_range` baru diketahui saat candle itu selesai, audit recovery dimulai dari **candle 5 menit berikutnya**;
4. `eventual_E20 = True` hanya jika high menyentuh E20 pada candle setelah `back_in_range` dan sebelum execution end;
...
Jika candle `back_in_range` sendiri juga sempat menyentuh E20 intrabar, touch itu **tidak** boleh dihitung sebagai recovery setelah retest karena urutan intrabar tidak diketahui.

## Kedalaman retest
Untuk tiap kasus hitung minimum low mulai dari candle `back_in_range` (untuk menangkap kedalaman candle konfirmasi retest itu sendiri) sampai:
...
- median minutes from back-in-range close to eventual E20;
