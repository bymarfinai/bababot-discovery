"""BTC Temporal A5.2 — conditional RUNNER vs PROTECT after +0.50% short MFE.

Frozen parent: Tuesday 06:00 SELL, TP 1.35%, SL 0.80%, max hold 6h.
Frozen management hinge from A5.1: first touch of +0.50% favorable excursion.
At that hinge, after the trigger 5m candle COMPLETES, decide at next 5m open:
- RUNNER: leave parent unchanged toward 1.35/0.8/6h.
- PROTECT: protect +0.20% from entry.

Important execution correction vs the coarse A5.1 probe:
If the decision-time open has already retraced through the +0.20% lock, exit at
that actual open instead of assuming an impossible fill back at +0.20%.
If still beyond +0.20% favorable, install the +0.20% stop from the decision bar.

No initial Tuesday entry is filtered. Configuration selection uses first 83
Tuesdays; last 56 are validation. Only interpretable failure states around the
+0.50% hinge are tested; no generic ML/feature zoo.
"""
...
def trigger_state(rows,i):
    """First bar touching +0.50% short MFE while parent remains alive; decide next open."""
    ...
    for j in range(i,end):
        ...
        # Parent ambiguity adverse-first. If parent closes, no management decision.
        if hit_tp and hit_sl:return None
        if hit_sl:return None
        if hit_tp:return None
        mfe=max(mfe,100*(e-x[3])/e); mae=max(mae,100*(x[2]-e)/e)
        ...
        if x[3]<=hinge:
            ...
            dec_open=rows[dec][1]
            hi=max(z[2] for z in rows[i:j+1]); lo=min(z[3] for z in rows[i:j+1])
            close=x[4]
            progress=100*(e-close)/e  # positive is still favorable to short at trigger close
            dec_progress=100*(e-dec_open)/e
            close_pos=(close-x[3])/max(x[2]-x[3],1e-9)
            path_abs=sum(abs(z) for z in rets)
            efficiency=abs(100*(e-close)/e)/max(path_abs,1e-9)
            c=context(rows,i)
            return {
              'trigger_i':j,'decision_i':dec,'time_min':(j-i+1)*5,
              'progress_close':progress,'progress_decision':dec_progress,
              'mfe':mfe,'mae':mae,'taker_avg':statistics.mean(takers),
              'taker_last':takers[-1],'close_pos_trigger':close_pos,
              'up_frac':up/len(rets),'efficiency':efficiency,
              'range_ratio':(x[2]-x[3])/max(pre_rng,1e-9),
              'volume_ratio':x[6]/max(pre_vol,1e-9),
              'day_pos':c['day_pos'] if c else None,
              'pre1':c['pre1'] if c else None,'pre4':c['pre4'] if c else None,
              'pre24':c['pre24'] if c else None,
            }
    return None

def protect_result(rows,i,s):
    """At decision open either market-exit if lock already lost, or arm lock stop."""
    e=rows[i][1]; pstop=e*(1-LOCK/100); tp=e*(1-TP/100); orig_sl=e*(1+SL/100)
    j=s['decision_i']; end=min(len(rows),i+HOLD//5); op=rows[j][1]
    # If +0.20 lock is already retraced through, only executable exit is current open.
    if op>=pstop:
...
def summarize(rs,key='final'):
    ...
    return {'trades':n,'wins':w,'losses':n-w,'wr':rnd(100*w/n,2),'pnl':rnd(sum(ps),3),'exp':rnd(sum(ps)/n,4),
            'pf':rnd(pos/neg,3) if neg else None,'mdd':rnd(max_drawdown(ps),3),'ls':loss_streak(ps),'blocks_pos':sum(x>0 for x in blocks)}
...
# Interpretable giveback-risk rules. Thresholds are small local grids, not arbitrary all-feature search.
def rule(name,s,p):
    ...
    if name=='WEAK_CLOSE_BUYERS': return s['progress_close']<=wc and s['taker_avg']>=tk
    ...
    if name=='BUYERS_REJECTION': return s['progress_close']<=wc and s['taker_avg']>=tk and s['close_pos_trigger']>=cp
    return False
...
def configs():
    ...
    for wc in (0.15,0.20,0.25,0.30,0.35,0.40):
        ...
        for tk in (-0.01,0.0,0.01): out.append(('WEAK_CLOSE_BUYERS',{'weak_close':wc,'taker':tk}))
        for cp in (0.55,0.65,0.75): out.append(('TRIGGER_REJECTION',{'weak_close':wc,'closepos':cp}))
        ...
        for tk in (0.0,0.01):
            for cp in (0.55,0.65): out.append(('BUYERS_REJECTION',{'weak_close':wc,'taker':tk,'closepos':cp}))
    # dedup exact dictionaries
    seen=set(); z=[]
    for n,p in out:
...
def atlas(recs):
    ...
    # action oracle label: which action makes more money per trade
    pb=[r for r in trig if r['protect']>r['base']]
    rb=[r for r in trig if r['base']>=r['protect']]
    ...
    return {'hinge_trades':len(trig),'protect_better':grp(pb),'runner_better':grp(rb),
            'protect_all_realistic':evaluate(recs,'WEAK_CLOSE',{'weak_close':999.0})}
...
def main():
    ...
    # Primary selection: preserve parent economics first, then improve WR; damaged winners penalized.
    ...
    # Strict cross-period: positive delta in BOTH periods and higher WR full. Descriptive robustness shortlist.
    cross=[]
    for n,p,d in grid:
    ...
    # WR-frontier candidates that retain at least 90% of parent PnL full, regardless split, for diagnosis only.
    frontier=[]
    for n,p,d in grid:
        v=evaluate(val,n,p); f=evaluate(recs,n,p)
    ...
    out={'status':'A52_RUNNER_VS_PROTECT','parent':{'tp':TP,'sl':SL,'hold_min':HOLD,'hinge':HINGE,'lock':LOCK},
         'data':{'tuesdays':len(recs),'discovery':len(disc),'validation':len(val),'configs':len(grid)},'baseline':base,
