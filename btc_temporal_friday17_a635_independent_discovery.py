"""BTC Friday17 A6.35 — independent temporal discovery.

No Friday15/16 entry or management rules are reused for discovery.
...
def main():
    ...
    for x in rows:
        if EVAL_START<=x[0]<EVAL_END:
            d=ldt(x[0])
            if d.weekday()==4 and d.hour==17 and d.minute==0: idx.append(im[x[0]])
    ...
    for hold in HOLDS:
        for tp in TPS:
            for sl in SLS:
                z=summarize_cfg(rows,idx,tp,sl,hold,split)
    ...
    out={'status':'FRIDAY17_A635_INDEPENDENT_DISCOVERY','data':{'fridays':len(idx),'discovery_n':split,'validation_n':len(idx)-split,
         'entry':'Friday 17:00 WIB exact open','fee_pct':a60.FEE_PCT,'notional_usd':a60.NOTIONAL,'configs':len(results)},
