...
while each leg is actually open, using the same A7.3 approximation.
"""
import json
...
def funding_pnl(funding,tsmap,entry_ts,exit_ts,qty,side):
    ...
    for ft,rate in funding:
        ...
        if ft>exit_ts: break
        px=(tsmap.get(ft) or [None,None])[1]
...
def parent_trade(rows,i,funding,tsmap):
    ...
    fp,nev=funding_pnl(funding,tsmap,t['ts'],exit_ts,qty,'BUY')
...
def close_buy(rows,i,j,funding,tsmap):
    ...
    gross=100*(px-e)/e
    qty=NOTIONAL/e
    fp,_=funding_pnl(funding,tsmap,rows[i][0],rows[j][0],qty,'BUY')
...
def short_leg(rows,j,end_i,tp,sl,funding,tsmap):
    ...
    gross=100*(e-ex)/e
    qty=NOTIONAL/e
    fp,_=funding_pnl(funding,tsmap,rows[j][0],rows[ex_i][0],qty,'SHORT')
...
def main():
      ...
      for fam in FAMILIES:
       for p in PROG:
        for t in TAKER:
          ...
          # Deduplicate semantics: families without P or T should not repeat irrelevant thresholds.
          if 'P' not in fam and p!=PROG[0]:continue
          if 'T' not in fam and t!=TAKER[0]:continue
          dcut=apply(rows,disc,tsmap,funding,c,'CUT')
          if dcut['actions']>=MIN_DISC_ACTIONS:
              cands.append({**c,'mode':'CUT','discovery':dcut})
    ...
    # Frozen selection is discovery-only. Favor PnL improvement, then PF, then fewer damaged winners.
    ...
    # Also return top discovery candidates for audit, evaluated OOS after ranking is frozen.
    audit=[]
