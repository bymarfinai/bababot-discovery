...
def _ensure_experiments_table():
    ...
    conn.execute("""CREATE TABLE IF NOT EXISTS mode3_experiments (
...
def _log_experiment(config, result, symbol, timeframe, days):
    try:
        ...
        s = result['summary']; pt = result.get('per_tool', {})
        sw = pt.get('SIDEWAYS', {}); bl = pt.get('BULL', {}); br = pt.get('BEAR', {})
        conn = sqlite3.connect(DB_PATH)
        conn.execute("""INSERT INTO mode3_experiments (
                timestamp, version, symbol, timeframe, days, cap_pct, tp_pct, va_window,
                entry_usd, leverage, fee_pct, slippage_pct, total_trades, wins, losses,
...
def compute_mtf_sideways_entry(rows_1h, rows_15m, vahs, vals):
    ...
    for i, r in enumerate(rows_1h):
        t_1h = r[0]; vah = vahs[i]; val = vals[i]
...
def compute_htf_4h_slope(ema_4h_series, window_bars=20):
    ...
    for i in range(window_bars, len(ema_4h_series)):
        curr = ema_4h_series[i]; prev = ema_4h_series[i - window_bars]
...
def compute_htf_4h_downtrend(close_series, ema_series, slope_series, regime_bars=3, slope_max=-0.3):
    n = len(close_series); downtrend = [False] * n
    for i in range(regime_bars, n):
        all_below = True
        for k in range(regime_bars):
            c = close_series[i - k]; e = ema_series[i - k]
            if c is None or e is None or c >= e: all_below = False; break
...
def classify_balance_position(entry_price, vah, val, boundary_pct=0.005):
    if vah is None or val is None or entry_price is None: return 'UNKNOWN'
    if entry_price > vah: return 'ABOVE'
    if entry_price < val: return 'BELOW'
    dist_to_vah = (vah - entry_price) / vah
    dist_to_val = (entry_price - val) / val
    if dist_to_vah < boundary_pct: return 'NEAR_VAH'
    if dist_to_val < boundary_pct: return 'NEAR_VAL'
    return 'INSIDE'
...
def backtest_mode3(
    ...
    config = Mode3Config(
        va_window=va_window, tp_pct=tp_pct, entry_usd=entry_usd, leverage=leverage,
    ...
    for i in range(len(rows)):
        vah, val, poc = vahs[i], vals[i], pocs[i]
    ...
    def get_htf_at_bar(bar):
        ...
        return (round(ema_v, 2) if ema_v else None, round(slope_v, 3) if slope_v is not None else None,
                round(close_v, 2) if close_v else None, round(vah_v, 2) if vah_v else None,
    ...
    for t in trades:
        ...
        trade_dict = {"tool": t.tool, "side": t.side,
        ...
        if htf_ema is not None:
            ...
            trade_dict["htf_4h_slope_pct"] = htf_slope; trade_dict["htf_4h_vah"] = htf_vah
            trade_dict["htf_4h_val"] = htf_val
        trade_list.append(trade_dict)

    result = {"symbol": symbol, "timeframe": timeframe, "days": days, "end_days_ago": end_days_ago,
        ...
        "summary": {"total_trades": n, "win_rate_pct": round(wr, 2),
            "wins": len(wins), "losses": len(losses),
            "total_pnl_usd": round(total_pnl_usd, 2), "total_pnl_pct": round(total_pnl_pct, 3),
