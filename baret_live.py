...
class ExchangeClient:
    ...
    def get_position(self, symbol, retries=3):
        """Get position. Returns dict if open, None if flat. Retries on API error."""
        last_err = None
                ...
                if isinstance(positions, list):
                    for p in positions:
                        if p["symbol"] == symbol and float(p.get("positionAmt", 0)) != 0:
                            return p
                return None  # API succeeded, position is flat
            except Exception as e:
                last_err = e
                _log(f"  ⚠️ get_position {symbol} (attempt {attempt+1}/{retries}): {e}")
                if attempt < retries - 1:
                    time.sleep(2 * (attempt + 1))
        # All retries exhausted — raise so callers can distinguish from flat
        raise ConnectionError(f"get_position {symbol} failed after {retries} retries: {last_err}")
...
def _detect_exchange_close(client, symbol, pos_info, configs, state, prefix, acct_name):
    ...
    try:
        try:
            ...
            _log(f"{prefix}  ⚠️ {symbol}: get_position API error, skipping check: {api_err}")
            return False  # Don't treat API error as position closed
