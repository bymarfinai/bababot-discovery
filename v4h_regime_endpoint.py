"""4H Regime Experiment — multiple EMA configs + simple price-above-EMA regime.

regime_mode:
  swing = ATR-scaled swing structure (current)
  ema_simple = close > both EMAs = BULL, close < both = BEAR

GET /v4h/quality?symbol=SOLUSDT&days=971&ema_4h_fast=7&ema_4h_slow=20&regime_mode=swing
GET /v4h/quality?symbol=SOLUSDT&days=971&ema_4h_fast=21&ema_4h_slow=50&regime_mode=swing
GET /v4h/quality?symbol=SOLUSDT&days=971&ema_4h_fast=7&ema_4h_slow=20&regime_mode=ema_simple
"""
import os,sqlite3,numpy as np,math,random,traceback
...
def _build_4h(rows_1h):
    ...
    for r in rows_1h:
        ts=r[0]
        if buf and (ts//block)!=(buf[0][0]//block):
            o4=buf[0][1];h4=max(x[2] for x in buf);l4=min(x[3] for x in buf)
            c4=buf[-1][4];v4=sum(x[5] for x in buf)
            candles.append((buf[0][0],o4,h4,l4,c4,v4,buf[-1][0]))
            buf=[]
        buf.append(r)
    if len(buf)==4:
        candles.append((buf[0][0],buf[0][1],max(x[2] for x in buf),min(x[3] for x in buf),buf[-1][4],sum(x[5] for x in buf),buf[-1][0]))
    return candles
...
class SwingRegime:
    ...
    def process(self,i,H,L,C,ef,es,atr):
        ...
        am=self.sa*atr[i] if atr[i]>0 else 0
        ...
        if self.hh>=2 and self.hl>=2 and bull_ema:return "BULL"
        if self.lh>=2 and self.ll>=2 and bear_ema:return "BEAR"
        return "SIDEWAYS"
...
def v4h_quality(
    ...
    try:
        ...
        T4=[c[0] for c in c4h];T4_last=[c[6] for c in c4h];n4=len(c4h)
        ...
        T1=[r[0] for r in r1h];n1=len(r1h);ef1=_ema(C1,ema_1h)
        ...
        if regime_mode=="swing":
            ...
            # Simple: close > both EMAs = BULL, close < both = BEAR
            for i in range(n4):
        ...
        for mode,entries in modes.items():
            ...
            for fn,fl in[("F1",[e for e in entries if e["j"]<third]),("F2",[e for e in entries if third<=e["j"]<2*third]),("F3",[e for e in entries if e["j"]>=2*third])]:
                if not fl:wf[fn]={"n":0,"ret_med":0};continue
        ...
        return{
            "symbol":symbol,"days":days,"n_4h":n4,
            "config":{"ema_fast":ema_4h_fast,"ema_slow":ema_4h_slow,"regime_mode":regime_mode},
            "regime_pct":dist_pct,"transitions":trans,
            "results":results,
        }
