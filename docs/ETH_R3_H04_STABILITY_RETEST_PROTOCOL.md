# ETH R3 H04 Stability Retest Protocol
...
This retest uses practical stability rather than exact cross-year invariance. The question is whether an H04 edge remains recognizable after a normal year-to-year perturbation, not whether the exact optimum is mathematically identical.
...
If `STABLE_EDGE_FROZEN`, freeze character rule, Development coordinate, chosen local 2023 coordinate, allowed one-step timing corridor, and all Development/Test metrics before opening 2024.
...
This protocol distinguishes normal degradation from collapse. Development WR 60%, Exp +$2.00, PF 1.60 followed by Test WR 57–58%, Exp +$1.3–$1.8, PF 1.3–1.5 is acceptable when local timing stability remains present. A Development edge that becomes negative, loses most expectancy, or requires a distant timing jump is not robust.

2022–2023 are internal robustness partitions, not pristine OOS. 2024 remains the one-shot confirmatory partition for this R3 protocol; 2025+ remains closed final OOS/reference.