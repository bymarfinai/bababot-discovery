"""
bull_tool.py — v1.6: multi-candle pullback + EMA20 reject
======================================================
FIX (per user insight):
- Peak must be at least peak_min_distance candles BEFORE current
- Pullback measured across multiple candles (peak → lowest_since_peak)
- Current candle must be EMA20 reject (touch + close above)
- NO entry at HH breakout candle itself
"""
...
class BullConfig:
    ...
    peak_min_distance: int = 2  # v1.6: peak must be at least N candles before current
...
def detect_bull_entry_signal(highs, lows, closes, opens, ema20, i, cfg):
    """
    v1.6: multi-candle pullback + EMA20 reject.
    - Peak must be at least peak_min_distance candles BEFORE current (not on current)
    - Pullback = (peak_high - lowest_since_peak) / peak_high
    - Current candle: touch EMA20 + close above (reject)
    """
    min_lookback = cfg.lookback_recent_high + cfg.peak_min_distance
    ...
    # v1.6: peak window EXCLUDES last peak_min_distance candles
    peak_end = i - cfg.peak_min_distance
    peak_start = i - cfg.lookback_recent_high
    if peak_end <= peak_start:
        return None

    peak_window = highs[peak_start:peak_end]
    recent_high = float(np.max(peak_window))
    peak_offset = int(np.argmax(peak_window))
    peak_idx = peak_start + peak_offset

    # Context: uptrend (peak > EMA20)
    if recent_high <= cur_ema:
        return None

    # v1.6: multi-candle pullback from peak
    if peak_idx + 1 > i:
        return None
    lowest_since_peak = float(np.min(lows[peak_idx + 1:i + 1]))
    pullback_pct = (recent_high - lowest_since_peak) / recent_high
    if pullback_pct < cfg.min_pullback_pct:
        return None

    # Current candle: EMA20 reject
    h_i = float(highs[i])
    l_i = float(lows[i])
    c_i = float(closes[i])
    o_i = float(opens[i])
    ...
    if cfg.require_close_confirm and c_i <= cur_ema:
        return None
    if c_i <= o_i:  # bullish body
        return None

    return BullSignal(
        idx=i, entry_price=c_i, ema20_at_entry=cur_ema,
        recent_high=recent_high,
        reason=f"peak_at_{i-peak_idx}c_ago pullback={pullback_pct*100:.2f}%",
    )
