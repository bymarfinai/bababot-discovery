...
OUT_MD = ROOT / 'BTC_F85_LONG_F15_SHORT_COLLISION_B27DS_Result.md'
OUT_SUM = ROOT / 'BTC_F85_LONG_F15_SHORT_COLLISION_B27DS_Summary.csv'
OUT_DETAIL = ROOT / 'BTC_F85_LONG_F15_SHORT_COLLISION_B27DS_Detail.csv'
OUT_STATUS = ROOT / 'BTC_F85_LONG_F15_SHORT_COLLISION_B27DS_Status.txt'
...
def build_long(x5):
    ...
    locked_parts = []
    for part in PARTS:
        z = dq.dn.dl.dg.lock(hybrid[hybrid.partition == part].copy(), f'B27DS_LONG_BASE_{part}')
...
def build_shorts(x5):
    ...
    for anchor in anchors:
        for cm in sorted(set(CLOCKS.values())):
            ...
            if r is not None:
                rows.append(r)
    if not rows: raise RuntimeError('no B27DS short cases')
...
def summarize_set(set_name, clock_tuple, short_all, raw_long_norm, baseline_long_norm, baseline_net):
    ...
    lp_prefilter, lp_lock, lp_acc, blocked_long, blocked_short = long_protected(shorts, baseline_long_norm, set_name)
...
def main():
    ...
    lines = [
        '# B27DS — F85 LONG + F15 SHORT Collision / Portfolio Interference Audit — Result','',
    ...
    lines += ['', '## Mechanical readout','',
              ...
              'Guardrail: the six SHORT clocks were selected after B27DR inspection, so B27DS is exploratory historical portfolio-interference evidence, not pristine OOS validation.', '',
              'Research only; live BBC unchanged.']
    OUT_MD.write_text('\n'.join(lines) + '\n')
    OUT_STATUS.write_text('B27DS_COLLISION_AUDIT_COMPLETED\n')
    print('\n'.join(lines))
