...
def load_candidates():
    ...
    # Same entry identity across candidate lanes.
...
def frozen_scores(x5):
    ...
    for cp,short in [('PLUS10','p10'),('PLUS15','p15')]:
        ...
        nums=cv.num_cols(cp)
        # Critical B27DC correction: score ALL fills, not future-label-gated model_eligible rows.
    ...
    return d[['event_id','partition','clock_block','regime','label','decision_ts','p10','p15','max_bull_body_r4',
              'GLOBAL_PLUS15_SAFE','PERSIST_10_15','REFINED_BULL_IMPULSE']],float(s10.development_auc),float(s15.development_auc)


def short_net(entry,exit_px):
    gross=(entry-exit_px)/entry
    return gross*NOTIONAL-FEE
...
def apply_rule(x5,cands,scores,rule):
    ...
    if rule=='NO_ABORT': raw=pd.Series(False,index=d.index)
    else: raw=d[rule].astype(bool)
    ...
    idx=d.index[d.abort]
    for i in idx:
        t=pd.Timestamp(d.at[i,'decision_ts'])
        assert t in x5.index,t
        d.at[i,'abort_ts']=t
        d.at[i,'abort_px']=float(x5.loc[t,'open'])
    d['adj_exit_ts']=d.exit_ts
    d['adj_exit_px']=pd.to_numeric(d.exit_px,errors='coerce')
    d['adj_exit_reason']=d.exit_reason.astype(str)
    if len(idx):
        d.loc[idx,'adj_exit_ts']=d.loc[idx,'abort_ts']
        d.loc[idx,'adj_exit_px']=d.loc[idx,'abort_px']
        d.loc[idx,'adj_exit_reason']='DETECTOR_ABORT_'+rule
    d['adj_net_pnl']=pd.to_numeric(d.net_pnl_usd,errors='coerce')
    if len(idx):
        d.loc[idx,'adj_net_pnl']=[short_net(float(d.at[i,'entry_px']),float(d.at[i,'abort_px'])) for i in idx]
    d['rule']=rule
    return d
...
def econ(g):
    ...
    span=(pd.Timestamp(t.fill_ts.max())-pd.Timestamp(t.fill_ts.min()))/pd.Timedelta(days=7)
    tpw=n/span if span>0 else np.nan
    return {'trades_n':n,'wr':float(np.mean(net>0)),'pf':pf,'expectancy':float(np.mean(net)),'total_net':float(np.sum(net)),
            'avg_win':float(np.mean(pos)) if len(pos) else np.nan,'avg_loss':float(np.mean(neg)) if len(neg) else np.nan,
            'max_dd':max_dd(net),'max_loss_streak':max_loss_streak(net),'abort_n':int(t.abort.sum()),'abort_rate':float(t.abort.mean()),'trades_per_week':float(tpw) if pd.notna(tpw) else np.nan}
...
def main():
    ...
    for cand in CANDS:
        ...
        for rule in RULES:
            allrows.append(apply_rule(x5,z,scores,rule))
    allout=pd.concat(allrows,ignore_index=True)
    allout.to_csv(OUT_TRADES,index=False)

    # Exact baseline reproduction before interpreting abort economics.
    b=allout[(allout.candidate.eq('BASE_H'))&(allout.rule.eq('NO_ABORT'))]
    bm=econ(b)
    assert bm['trades_n']==652
    assert abs(bm['total_net']-(-278.39))<0.03,(bm['total_net'])

    rows=[]
    scopes=[('external',lambda z:z.partition.eq('external')),('development',lambda z:z.partition.eq('development')),
            ('reference_validation',lambda z:z.partition.eq('reference_validation')),
            ('POOLED_REUSED_EXTVAL',lambda z:z.partition.isin(['external','reference_validation'])),('POOLED_MAJOR',lambda z:pd.Series(True,index=z.index))]
    for cand in CANDS:
        for rule in RULES:
            z=allout[(allout.candidate.eq(cand))&(allout.rule.eq(rule))]
            for name,fn in scopes:
                rows.append({'candidate':cand,'rule':rule,'scope':name,**econ(z[fn(z)])})
    ...
    for cand in CANDS:
        for rule in RULES[1:]:
            ...
            for lab in ('BAD','GOOD','OTHER'):
                attrs.append({'candidate':cand,'rule':rule,'base_label':lab,'abort_n':int((z.label_score.astype(str)==lab).sum())})
    ...
    lines=['# B27DC — BTC 24H F05 SHORT Causal Abort Economics — Result','',
           ...
           f'Parent reproduction: +10 AUC **{auc10:.10f}**, +15 AUC **{auc15:.10f}**; BASE_H no-abort total net **{money(bm["total_net"])}**.','',
    ...
    lines += ['', '## Pooled and partition economics','',
    ...
    for cand in CANDS:
        for rule in RULES[1:]:
            q=attr[(attr.candidate.eq(cand))&(attr.rule.eq(rule))]
            vals={r.base_label:int(r.abort_n) for r in q.itertuples(index=False)}
            lines.append(f'| {cand} | {rule} | {vals.get("BAD",0)} | {vals.get("GOOD",0)} | {vals.get("OTHER",0)} |')
    lines += ['', '**Frozen status: `B27DC_CAUSAL_ABORT_ECON_RESEARCH_ONLY_NO_LIVE_PROMOTION`.**','',
