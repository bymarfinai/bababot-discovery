...
ANCHOR_HOUR = 4
PFX = 'BNB_HOURLY_STRUCTURE_B27FE'
...
def build_sessions(x5):
    ...
    for d in pd.date_range(local_first, local_last, freq='D'):
        ...
        if len(ref) != 48 or len(exe) != 48:
            raise AssertionError(f'incomplete B27FE session {day}: ref={len(ref)}/48 exe={len(exe)}/48')
    ...
    d = pd.DataFrame(rows)
    if d.empty:
        raise AssertionError('no B27FE sessions')
    return d
...
def main():
    if not (ROOT / f'{PFX}_Preregistration.md').exists():
        raise AssertionError('B27FE preregistration missing')
    ...
    lines = [
        ...
        f"- 04:00 WIB: **{pooled['causal_leave']} leaves / {pooled['h2']} H2 = {pct(pooled['h2_rate'])}**",
        f'- B27FE minus prior leader ({leader_slot}) H2-rate delta: **{delta_pp:+.1f} percentage points**','',
        '## Weekday breakdown','',
    ...
    OUT_MD.write_text('\n'.join(lines) + '\n', encoding='utf-8')
    OUT_STATUS.write_text('B27FE_BNB_HOUR04_STRUCTURE_COMPLETE\n', encoding='utf-8')
    print('\n'.join(lines))
