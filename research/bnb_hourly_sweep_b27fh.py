...
def build_sessions(x5, anchor_hour):
    ...
    for d in pd.date_range(local_first, local_last, freq='D'):
        ...
        if len(ref) != 48 or len(exe) != 48:
            raise AssertionError(
                f'incomplete B27FH session anchor={anchor_hour:02d} day={day}: '
    ...
    d = pd.DataFrame(rows)
    if d.empty:
        raise AssertionError(f'no B27FH sessions for anchor {anchor_hour:02d}')
    return d
...
def main():
    if not (ROOT / f'{PFX}_Preregistration.md').exists():
        raise AssertionError('B27FH preregistration missing')
    ...
    lines += ['', '## Frozen comparison: 00:00–13:00 WIB', '',
    ...
    lines += [
        '',
        f'Current structural leader after B27FH: **{leader_h:02d}:00 WIB — {leader_rate * 100:.1f}% H2/leave ({leader_h2}/{leader_lv})**.',
        '',
    ...
    OUT_MD.write_text('\n'.join(lines) + '\n', encoding='utf-8')
    OUT_STATUS.write_text('B27FH_BNB_HOUR10_13_SWEEP_COMPLETE\n', encoding='utf-8')
    print('\n'.join(lines))
