...
PFX='ETH_B27DX_S7B_POST_LEAVE_RETRACE_COMPRESSION'
...
def attach_compression(c,x):
    q=c.copy(); rows=[]
    for r in q.itertuples(index=False):
        es=pd.Timestamp(r.execution_start); ee=es+pd.Timedelta(minutes=HORIZON_MIN)
        ...
        if w is None or not bool(w.get('clean',False)):
            raise AssertionError(f'candidate lost causal window {r.partition} {r.execution_utc} {es}')
        leave=pd.Timestamp(w['leave_bar']); eligible=pd.Timestamp(w['eligible_start']); entry=pd.Timestamp(r.entry_bar_start)
        if eligible != leave + pd.Timedelta(minutes=5): raise AssertionError('eligible chronology mismatch')
        if entry < eligible: raise AssertionError('entry before eligible start')
        delay=float((entry-eligible)/pd.Timedelta(minutes=1)); capacity=float((ee-eligible)/pd.Timedelta(minutes=1))
        if capacity <= 0 or delay < 0 or delay > capacity+1e-9: raise AssertionError('invalid post-leave timing')
        frac=delay/capacity
        rows.append({'partition':r.partition,'exec_min':int(r.exec_min),'execution_start':es,'entry_bar_start':entry,
                     'leave_bar_start':leave,'eligible_start':eligible,'execution_end':ee,
                     'retrace_delay_min':delay,'post_leave_capacity_min':capacity,'retrace_fraction':frac,
                     'compression_known_by_entry':bool(leave < eligible <= entry <= ee)})
    f=pd.DataFrame(rows)
    keys=['partition','exec_min','execution_start','entry_bar_start']
    if f.duplicated(keys).any(): raise AssertionError('duplicate compression feature keys')
    q=q.merge(f,on=keys,how='left',validate='one_to_one')
    if q.retrace_fraction.isna().any(): raise AssertionError('missing compression features')
    return q,f

def filt_mask(g): return g.retrace_fraction <= .50
...
def score(c):
    ...
    for ex in CLOCKS:
        for p in PARTS:
            ...
            for name,f in [('BASE',raw),('FAST_POST_LEAVE_HALF',raw[filt_mask(raw)].copy())]:
...
def choose(summary):
    rows=[]
    for ex in CLOCKS:
        d=summary[(summary.exec_min==ex)&(summary.partition=='development')&(summary['filter']=='FAST_POST_LEAVE_HALF')].iloc[0]
        prom=bool(d.n>=20 and d.retention>=.50 and d.wr>=.75 and d.pf>=1.50 and d.expectancy>=.80 and d.net>0)
        row={'exec_min':ex,'execution_utc':cl(ex),'selected_filter':'FAST_POST_LEAVE_HALF' if prom else '',
        ...
        if prom:
            ...
            for p in ('external','reference_validation'):
                z=summary[(summary.exec_min==ex)&(summary.partition==p)&(summary['filter']=='FAST_POST_LEAVE_HALF')].iloc[0]
...
def main():
    x,cov=s7a.s4.b.m.m.load5(); base,a0=s7a.feature_candidates(x); c,af=attach_compression(base,x); c.to_csv(OUT_CAND,index=False)
    audit=pd.concat([a0.reset_index(drop=True),af[['compression_known_by_entry']].reset_index(drop=True)],axis=1);audit.to_csv(OUT_AUDIT,index=False)
    causal=bool(audit.feature_known_by_entry.all() and audit.compression_known_by_entry.all()) if len(audit) else False
    ...
    if not causal:status='ETH_S7B_CAUSAL_AUDIT_FAILED'
    elif ndev==0:status='ETH_S7B_NO_DEV_COMPRESSION_FILTER'
    elif nrep==0:status='ETH_S7B_DEV_FILTERS_NOT_REPLICATED'
    else:
        ...
        status='ETH_S7B_COMPRESSION_PORTFOLIO_BTC_QUALITY_SUPPORTED' if btc else 'ETH_S7B_COMPRESSION_FILTERS_REPLICATED_BELOW_BTC'
    lines=['# ETH B27DX — S7B Post-Leave Retrace Compression — Result','',f'ETH raw 5m coverage: **{cov:.4%}**.','',f'- Causal audit: **{"PASS" if causal else "FAIL"}**.','- Frozen filter: **FAST_POST_LEAVE_HALF = F75 fill occurs within the first half of the remaining execution opportunity after causal leave completion.**','',
           '## Development comparison','', '| Clock | Variant | N | Retain | WR | PF | Exp | Net | Promote |','|---:|---|---:|---:|---:|---:|---:|---:|---|']
    for ex in CLOCKS:
        for name in ('BASE','FAST_POST_LEAVE_HALF'):
    ...
    if nrep==0: lines.append('No Development-promoted compression filter replicated in both historical validation partitions.')
    else:
    ...
    lines += ['','## Decision','',f'**Status: {status}**','', '- No threshold sweep, geometry, runner, leverage, fee, or live-code change was made.']
