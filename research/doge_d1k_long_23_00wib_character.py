...
# pair-agnostic E12 engine. D1K changes only the clock habitat and labels.
import doge_d1d_long_16_17wib_character as frozen

engine = frozen.engine
ROOT = Path(__file__).resolve().parent.parent
PFX = "DOGE_D1K_LONG_23_00WIB_CHARACTER"
...
# Frozen D1K habitat only: 16:00..16:45 UTC = 23:00..23:45 WIB.
engine.CLOCKS = (960, 975, 990, 1005)
...
def main():
    ...
    lines = [
        "# DOGE D1K — 23:00–00:00 WIB LONG Character Discovery Result", "",
    ...
    if len(C) == 0:
        status = "DOGE_D1K_NO_LONG_CHARACTER"
        best = rank.iloc[0]
        OUT_STATUS.write_text(status + "\n")
        lines += [
            "", "## Formal verdict", "", f"**Status: {status}**", "",
            "No 23:00–00:00 WIB DOGE LONG candidate passed anchor + pooled + all-era gates simultaneously.", "",
    ...
    status = "DOGE_D1K_LONG_CHARACTER_FOUND"
    OUT_STATUS.write_text(status + "\n")
    lines += [
