...
PFX = "SOL_STRUCTURE_NATIVE_TRIGGER_V2"
YEARS = v1.YEARS
TRIGGER_MAX_BARS = 12
...
def emit(rows, s, trigger, signal_i, idx, op, hi, lo, cl, trigger_level, extra=None):
...
def apply_native_triggers(setups: pd.DataFrame, x5: pd.DataFrame) -> pd.DataFrame:
    ...
    rows = []
    allowed = {x[0] for x in DETECTORS}
    ...
    for _, s in setups.iterrows():
        ...
        if structure == "SWEEP_RECLAIM":
            ...
            trigger_level = float(hi[setup_i])
            for i in range(setup_i + 1, last_scan + 1):
                if cl[i] < anchor:
                    break
                if cl[i] > trigger_level:
                    emit(rows, s, "RECLAIM_CANDLE_HIGH_BREAK", i, idx, op, hi, lo, cl, trigger_level)
                    break

        elif structure == "HL_SETUP":
            hl = float(s.secondary_level)
            hl_pivot_i = int(s.secondary_pivot_i)
            if not np.isfinite(hl) or hl_pivot_i < 0:
                continue
            post_h = None
            for i in range(setup_i + 1, last_scan + 1):
                if cl[i] < hl:
                    break
                if post_h is None:
                    candidates = [h for h in highs_by_confirm.get(i, []) if int(h["pivot_i"]) > hl_pivot_i]
                    if candidates:
                        post_h = candidates[0]
                    continue
                level = float(post_h["price"])
                if cl[i] > level:
                    emit(rows, s, "FIRST_POST_HL_SWING_HIGH_BREAK", i, idx, op, hi, lo, cl, level,
                         {"post_h_pivot_i": int(post_h["pivot_i"]), "post_h_confirm_i": int(post_h["confirm_i"])})
                    break
            ...
            if pivot_i < 0 or pivot_i > setup_i:
                continue
            trigger_level = float(hi[pivot_i])
            for i in range(setup_i + 1, last_scan + 1):
                if cl[i] < breakout_level:
                    break
                if cl[i] > trigger_level:
                    emit(rows, s, "PULLBACK_PIVOT_HIGH_BREAK", i, idx, op, hi, lo, cl, trigger_level,
                         {"pullback_pivot_i": pivot_i})
                    break
    ...
    if out.empty:
        raise RuntimeError("no V2 native-trigger trades detected")
    ...
    for structure, trigger in DETECTORS:
        ...
        for year in YEARS:
            ...
            if ey["pnl_usd"] > 0:
                pos_years += 1
            yearly_rows.append({
                "detector": detector, "structure": structure, "trigger": trigger, "year": year,
                "n": ey["n"], "wr60": ey["wr"], "expectancy60_pct": ey["expectancy_pct"],
        ...
        pooled_rows.append({
            "detector": detector, "structure": structure, "trigger": trigger,
            "setup_n": setup_n, "triggered_n": e["n"],
...
def main():
    ...
    lines += ["## Interpretation rule", "",
              "PASS_TO_CHARACTERIZATION means this exact structure + V2 native trigger may advance to its own execution / TP / SL characterization.",
              "REJECTED_AS_DEFINED rejects only this exact V2 trigger for that frozen structure. Do not rescue it on 2020-2024 by retuning the trigger window, levels, hours, indicators, regime filters, TP or SL.", "",
              f"PASSING_NATIVE_DETECTORS={','.join(passing) if passing else 'NONE'}", ""]
