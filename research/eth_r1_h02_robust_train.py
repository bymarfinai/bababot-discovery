#!/usr/bin/env python3
"""ETH R1 H02 robust train discovery.
...
PFX = "ETH_R1_H02_ROBUST"

# The only scientific variable changed from H00/H01.
study.HOUR_WIB = 2
...
def main():
    ...
    if study.OUT_RESULT.exists():
        text = study.OUT_RESULT.read_text().replace("ETH R1 H00", "ETH R1 H02")
        study.OUT_RESULT.write_text(text)
    if study.OUT_STATUS.exists():
        text = study.OUT_STATUS.read_text().replace("ETH_R1_H00", "ETH_R1_H02")
