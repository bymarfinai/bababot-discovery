...
PFX = "BNB_ECONOMIC_FIRST_B28F_LONG_05_06WIB_CHARACTER"
...
def main():
    ...
    lines = [
        "# BNB Economic-First B28F — 05:00–06:00 WIB LONG Character Discovery Result", "",
        f"Raw BNBUSDT 5m coverage: **{coverage:.4%}**.",
        "Direction: **LONG only**.",
        "Time habitat: **05:00–06:00 WIB only** (22:00, 22:15, 22:30, 22:45 UTC anchors).",
        "Methodology and gates: **identical to preregistered B28A–B28E; only the hour changed**.",
    ...
    if len(C) == 0:
        status = "BNB_ECONOMIC_FIRST_B28F_NO_LONG_CHARACTER"
        OUT_STATUS.write_text(status + "\n")
        lines += ["", f"**Status: {status}**", "",
                  "No 05:00–06:00 WIB BNB LONG character passed the preregistered anchor-stability + pooled-economics/risk + all-era gates.",
                  "No gate relaxation, prior-hour rescue, B27 structural rescue, or OOS exposure is allowed.",
                  "Next preregistered habitat: **06:00–07:00 WIB** with the unchanged methodology.", "", "Research/shadow only."]
    ...
    status = "BNB_ECONOMIC_FIRST_B28F_LONG_CHARACTER_FOUND"
    OUT_STATUS.write_text(status + "\n")
