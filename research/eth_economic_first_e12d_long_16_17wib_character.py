...
PFX = "ETH_ECONOMIC_FIRST_E12D_LONG_16_17WIB_CHARACTER"
...
CLOCKS = (540, 555, 570, 585)  # 09:00..09:45 UTC = 16:00..16:45 WIB
...
def main():
    # Reuse frozen E12A grammar and gates; change only the one-hour habitat.
    e12a.CLOCKS = CLOCKS
    ...
    lines = [
        "# ETH Economic-First E12D — 16:00–17:00 WIB LONG Character Discovery Result", "",
        f"Raw ETHUSDT 5m coverage: **{coverage:.4%}**.",
        "Direction: **LONG only**.",
        "Time habitat: **16:00–17:00 WIB only** (09:00, 09:15, 09:30, 09:45 UTC anchors).",
        "Scientific grammar and gates: **identical to preregistered E12A/E12B/E12C; only the hour changed**.",
    ...
    if len(C) == 0:
        status = "ETH_ECONOMIC_FIRST_E12D_NO_LONG_CHARACTER"
        OUT_STATUS.write_text(status + "\n")
        lines += ["", f"**Status: {status}**", "",
                  "No 16:00–17:00 WIB LONG character passed the preregistered anchor-stability + pooled-economics + all-era gates.",
    ...
    status = "ETH_ECONOMIC_FIRST_E12D_LONG_CHARACTER_FOUND"
    OUT_STATUS.write_text(status + "\n")
    ...
    lines += ["", f"**Status: {status}**", "",
              "Discovery-only: OOS stays closed. Freeze this one character for a bounded follow-up; do not reopen the E12D search grid.", "", "Research/shadow only."]
    OUT_RESULT.write_text("\n".join(lines) + "\n")
    print(OUT_RESULT.read_text())
