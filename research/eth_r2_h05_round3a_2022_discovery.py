...
def eval_cell(cache, lb, hold, pair):
    ...
    half_ok=bool(
        h1["trades"]>=18 and h2["trades"]>=18 and
    ...
    supportive=bool(
        s["trades"]>=45 and finite(s["win_rate"]) and s["win_rate"]>=.52 and
        ...
        s["max_loss_streak"]<=10 and half_ok
    )
    eval_a=0; pos_a=0
    for clock in e12.CLOCKS:
        A=T[T.clock==clock]; a=summarize(A)
        ev=a["trades"]>=12
        pos=bool(ev and finite(a["expectancy"]) and a["expectancy"]>0 and finite(a["pf"]) and a["pf"]>1)
        eval_a+=int(ev); pos_a+=int(pos)
    strict=bool(
        supportive and s["trades"]>=60 and s["win_rate"]>=.55 and s["expectancy"]>=.50 and
        s["pf"]>=1.20 and s["max_dd"]<=110 and s["max_loss_streak"]<=8 and
        eval_a>=3 and pos_a>=3
    )
    return {
        "character_rule":f"DRIVE_DOWN__{pair}","pair_state":pair,"lookback_min":lb,"hold_min":hold,
        **s,
...
def main():
    ...
    # R3A cache has no 2023+ bars, guaranteeing 2023 cannot affect discovery.
    ...
    for pack in cache.values():
        ...
        down=masks["DRIVE_DOWN"]
        for pair in PAIR_STATES:
            masks[f"R3_{pair}"]=down & masks[pair]
    ...
    for pair in PAIR_STATES:
        ...
        for comp in r1.connected_components(coords):
            ...
            C=pd.DataFrame(C)
            lbs=sorted(set(int(x) for x in C.lookback_min)); holds=sorted(set(int(x) for x in C.hold_min))
            stricts=int(C.discovery_strict.astype(bool).sum())
            med_min_half=float(C.min_half_exp.median()); med_exp=float(C.expectancy.median()); med_pf=float(C.pf.median()); med_dd=float(C.max_dd.median())
            q=bool(len(C)>=4 and len(lbs)>=2 and len(holds)>=2 and stricts>=1 and med_exp>=.25 and med_pf>=1.10 and med_min_half>0)
            region_rows.append({"region_id":rid,"pair_state":pair,"character_rule":f"DRIVE_DOWN__{pair}","cells":len(C),
                                "lookbacks":";".join(map(str,lbs)),"holds":";".join(map(str,holds)),"strict_cells":stricts,
                                "median_min_half_exp":med_min_half,"median_exp":med_exp,"median_pf":med_pf,"median_dd":med_dd,"qualifies":q})
            region_cells[rid]=C
    ...
    Q=R[R.qualifies].copy()

    lines=["# ETH R2 H05 — Round 3A 2022 Nested Discovery","",
           "**DISCOVERY = 2022 ONLY. 2023 INTERNAL HOLDOUT UNOPENED. 2024 HARD LOCKED. OOS CLOSED.**","",
           f"Source coverage: {coverage:.4%}.",f"New grammar: {len(PAIR_STATES)} DRIVE_DOWN × pair-state interactions × 20 coarse timings = **{len(D)} cells**.",
           f"Supportive 2022 cells: **{int(D.discovery_supportive.sum())}**; strict cells: **{int(D.discovery_strict.sum())}**; qualifying regions: **{len(Q)}**.",""]
    if len(Q)==0:
        status="ETH_R2_H05_R3A_NO_2022_ROBUST_REGION"; OUT_STATUS.write_text(status+"\n")
        if OUT_LOCK.exists(): OUT_LOCK.unlink()
        lines += ["## Verdict","","**R3A_NO_2022_ROBUST_REGION**","","2023 remains unopened; no Round-3 interaction may be rescued using holdout knowledge."]
    else:
        ...
        status="ETH_R2_H05_R3A_2022_REGION_FROZEN"; OUT_STATUS.write_text(status+"\n")
        lines += ["## Frozen 2022 region","",f"Rule: **{reg.character_rule}**; region {int(reg.cells)} cells / {int(reg.strict_cells)} strict.",
                  f"Representative chosen by centrality: **LB{int(rep.lookback_min)} / H{int(rep.hold_min)}**; N {int(rep.trades)}, WR {100*float(rep.win_rate):.2f}%, exp ${float(rep.expectancy):+.2f}, PF {float(rep.pf):.3f}.",
