...
PFX = "ETH_ECONOMIC_FIRST_E12R_LONG_06_07WIB_CHARACTER"
...
def main():
    ...
    lines = [
        "# ETH Economic-First E12R — 06:00–07:00 WIB LONG Character Discovery Result", "",
        f"Raw ETHUSDT 5m coverage: **{coverage:.4%}**.",
        "Direction: **LONG only**.",
        "Time habitat: **06:00–07:00 WIB only** (23:00, 23:15, 23:30, 23:45 UTC anchors).",
        "Scientific grammar and gates: **identical to preregistered E12A–E12Q; only the hour changed**.",
    ...
    if len(C) == 0:
        status = "ETH_ECONOMIC_FIRST_E12R_NO_LONG_CHARACTER"
        OUT_STATUS.write_text(status + "\n")
        lines += ["", f"**Status: {status}**", "", "No 06:00–07:00 WIB LONG character passed the preregistered anchor-stability + pooled-economics + all-era gates.", "No gate relaxation and no OOS exposure.", "", "Research/shadow only."]
    ...
    status = "ETH_ECONOMIC_FIRST_E12R_LONG_CHARACTER_FOUND"
    OUT_STATUS.write_text(status + "\n")
