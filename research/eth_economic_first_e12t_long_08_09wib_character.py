...
PFX = "ETH_ECONOMIC_FIRST_E12T_LONG_08_09WIB_CHARACTER"
...
def main():
    ...
    lines = [
        "# ETH Economic-First E12T — 08:00–09:00 WIB LONG Character Discovery Result", "",
        f"Raw ETHUSDT 5m coverage: **{coverage:.4%}**.",
        "Direction: **LONG only**.",
        "Time habitat: **08:00–09:00 WIB only** (01:00, 01:15, 01:30, 01:45 UTC anchors).",
        "Scientific grammar and gates: **identical to preregistered E12A–E12S; only the hour changed**.",
    ...
    if len(C) == 0:
        status = "ETH_ECONOMIC_FIRST_E12T_NO_LONG_CHARACTER"
        OUT_STATUS.write_text(status + "\n")
        lines += ["", f"**Status: {status}**", "", "No 08:00–09:00 WIB LONG character passed the preregistered anchor-stability + pooled-economics + all-era gates.", "No gate relaxation and no OOS exposure.", "", "Research/shadow only."]
    ...
    status = "ETH_ECONOMIC_FIRST_E12T_LONG_CHARACTER_FOUND"
    OUT_STATUS.write_text(status + "\n")
