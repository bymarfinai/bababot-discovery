...
ANCHOR_HOUR = 5
PFX = 'BNB_HOURLY_STRUCTURE_B27FF'
...
def build_sessions(x5):
    ...
    for d in pd.date_range(local_first, local_last, freq='D'):
        ...
        if len(ref) != 48 or len(exe) != 48:
            raise AssertionError(f'incomplete B27FF session {day}: ref={len(ref)}/48 exe={len(exe)}/48')
    ...
    d = pd.DataFrame(rows)
    if d.empty:
        raise AssertionError('no B27FF sessions')
    return d
...
def main():
    if not (ROOT / f'{PFX}_Preregistration.md').exists():
        raise AssertionError('B27FF preregistration missing')
    ...
    prior = [
        ...
        ('03:00 WIB',142,96,96/142),
        ('04:00 WIB',142,108,108/142),
    ]
    leader_slot, _, _, leader_rate = max(prior, key=lambda x: x[3])
    delta_pp = 100.0 * (pooled['h2_rate'] - leader_rate)
    lines = [
        '# BNB Hour-by-Hour Structural Discovery — B27FF','',
        '**Anchor tested:** 05:00 WIB only.','',
        '- Reference: 05:00–09:00 WIB',
        '- Execution: 09:00–13:00 WIB',
        ...
        f"- 05:00 WIB: **{pooled['causal_leave']} leaves / {pooled['h2']} H2 = {pct(pooled['h2_rate'])}**",
        f'- B27FF minus prior leader ({leader_slot}) H2-rate delta: **{delta_pp:+.1f} percentage points**','',
        '## Weekday breakdown','',
    ...
    lines += ['', '## Interpretation','',
        'This milestone ranks only whether the 05:00 WIB clock geometry produces a repeatable LONG revisit structure under the same state machine as prior hourly milestones. The H2 rate is not trading WR and cannot be compared to TP/SL WR.','',
        '**Status: B27FF_BNB_HOUR05_STRUCTURE_COMPLETE**','',
        'STOP: do not test 06:00 WIB or define an entry in B27FF.'
    ]
    OUT_MD.write_text('\n'.join(lines) + '\n', encoding='utf-8')
    OUT_STATUS.write_text('B27FF_BNB_HOUR05_STRUCTURE_COMPLETE\n', encoding='utf-8')
    print('\n'.join(lines))
