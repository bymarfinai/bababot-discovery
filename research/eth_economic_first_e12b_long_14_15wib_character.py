...
PFX = "ETH_ECONOMIC_FIRST_E12B_LONG_14_15WIB_CHARACTER"
...
CLOCKS = (420, 435, 450, 465)  # 07:00..07:45 UTC = 14:00..14:45 WIB
...
def main():
    # Reuse the preregistered E12A scientific grammar and gates; change only the one-hour habitat.
    e12a.CLOCKS = CLOCKS
    ...
    lines = [
        "# ETH Economic-First E12B — 14:00–15:00 WIB LONG Character Discovery Result", "",
        f"Raw ETHUSDT 5m coverage: **{coverage:.4%}**.",
        "Direction: **LONG only**.",
        "Time habitat: **14:00–15:00 WIB only** (07:00, 07:15, 07:30, 07:45 UTC anchors).",
        "Scientific grammar and gates: **identical to preregistered E12A; only the hour changed**.",
    ...
    if len(C) == 0:
        status = "ETH_ECONOMIC_FIRST_E12B_NO_LONG_CHARACTER"
        OUT_STATUS.write_text(status + "\n")
        lines += ["", f"**Status: {status}**", "",
                  "No 14:00–15:00 WIB LONG character passed the preregistered anchor-stability + pooled-economics + all-era gates.",
    ...
    status = "ETH_ECONOMIC_FIRST_E12B_LONG_CHARACTER_FOUND"
    OUT_STATUS.write_text(status + "\n")
    ...
    lines += ["", f"**Status: {status}**", "",
              "Discovery-only: OOS stays closed. Freeze this one character for the next bounded follow-up; do not reopen the E12B search grid.", "", "Research/shadow only."]
    OUT_RESULT.write_text("\n".join(lines) + "\n")
    print(OUT_RESULT.read_text())
