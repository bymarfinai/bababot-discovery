...
PFX = "ETH_ECONOMIC_FIRST_E12P_LONG_04_05WIB_CHARACTER"
...
def main():
    ...
    lines = [
        "# ETH Economic-First E12P — 04:00–05:00 WIB LONG Character Discovery Result", "",
        f"Raw ETHUSDT 5m coverage: **{coverage:.4%}**.",
        "Direction: **LONG only**.",
        "Time habitat: **04:00–05:00 WIB only** (21:00, 21:15, 21:30, 21:45 UTC anchors).",
        "Scientific grammar and gates: **identical to preregistered E12A–E12O; only the hour changed**.",
    ...
    if len(C) == 0:
        status = "ETH_ECONOMIC_FIRST_E12P_NO_LONG_CHARACTER"
        OUT_STATUS.write_text(status + "\n")
        lines += ["", f"**Status: {status}**", "", "No 04:00–05:00 WIB LONG character passed the preregistered anchor-stability + pooled-economics + all-era gates.", "No gate relaxation and no OOS exposure.", "", "Research/shadow only."]
    ...
    status = "ETH_ECONOMIC_FIRST_E12P_LONG_CHARACTER_FOUND"
    OUT_STATUS.write_text(status + "\n")
