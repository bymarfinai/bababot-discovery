...
PRIOR = [
    (0,137,105),(1,162,132),(2,162,126),(3,142,96),(4,142,108),(5,141,94),
    (6,148,104),(7,149,114),(8,143,113),(9,161,118),(10,175,136),(11,159,120),
    (12,161,117),(13,183,139),(14,162,126),(15,178,132),(16,157,107),(17,142,94),
    (18,127,89),(19,133,91),(20,129,89),(21,145,107),
]
...
def build_sessions(x5, anchor_hour):
    ...
    for d in pd.date_range(local_first, local_last, freq='D'):
        ...
        if len(ref) != 48 or len(exe) != 48:
            raise AssertionError(f'incomplete B27FK session anchor={anchor_hour:02d} day={day}: ref={len(ref)}/48 exe={len(exe)}/48')
    ...
    d = pd.DataFrame(rows)
    if d.empty:
        raise AssertionError(f'no B27FK sessions for anchor {anchor_hour:02d}')
    return d
...
def main():
    if not (ROOT / f'{PFX}_Preregistration.md').exists():
        raise AssertionError('B27FK preregistration missing')
    ...
    for anchor in ANCHORS:
        ...
        summary_rows.append({'anchor_hour_wib': anchor, 'scope': 'POOLED', 'label': label_for(pooled), **pooled})
        for wd in weekdays:
            m = metrics(d[d.weekday == wd])
            summary_rows.append({'anchor_hour_wib': anchor, 'scope': wd, 'label': '', **m})
    ...
    lines = [
        '# BNB Hourly Structural Sweep — B27FK', '',
        '**Final anchors tested together under one preregistration:** 22:00, 23:00 WIB.', '',
        '- This completes the frozen 24-hour discovery clock sweep',
        ...
        '- Same B27EM/B27FA–B27FJ state machine for both clocks',
        '- No entry, TP, SL, PnL, fee, weekday filter, or holdout economics', '',
        '## Final batch pooled results', '',
    ...
    for h in ANCHORS:
        ...
        lines.append(f"| {h:02d}:00 WIB | {m['sessions']} | {m['k1_qualified']} | {m['causal_leave']} | {m['h2']} | {pct(m['h2_rate'])} | {m['opposite_break_before_h2']} | {m['no_h2_by_end']} | {pct(m['resolved_h2_share'])} | {med} | {label_for(m)} |")

    lines += ['', '## Final frozen comparison: 00:00–23:00 WIB', '',
    ...
    ranked = sorted(all_clock_rows, key=lambda x: x[3], reverse=True)
    lines += ['', f'Final structural leader after the full 24-hour sweep: **{leader_h:02d}:00 WIB — {leader_rate * 100:.1f}% H2/leave ({leader_h2}/{leader_lv})**.', '',
              '## Final ranking', '', '| Rank | Anchor | Leaves | H2 | H2/leave |', '|---:|---|---:|---:|---:|']
    for i, (h, lv, h2, rate) in enumerate(ranked, 1):
        lines.append(f'| {i} | {h:02d}:00 WIB | {lv} | {h2} | {pct(rate)} |')

    lines += ['', '## Per-anchor weekday breakdown']
    for h in ANCHORS:
    ...
    lines += ['', '## Interpretation', '',
              'B27FK completes the temporal habitat sweep only. H2/leave is a structural outcome rate, not trading win rate. No economic edge is claimed from these results alone.', '',
              '**Status: B27FK_BNB_FULL_24H_STRUCTURAL_SWEEP_COMPLETE**', '',
              'STOP: the 24-hour clock sweep is complete. Do not define an entry, select weekdays, normalize clocks, or reveal holdout data inside B27FK; those require a new preregistered milestone.']

    OUT_MD.write_text('\n'.join(lines) + '\n', encoding='utf-8')
    OUT_STATUS.write_text('B27FK_BNB_FULL_24H_STRUCTURAL_SWEEP_COMPLETE\n', encoding='utf-8')
    print('\n'.join(lines))
