...
PFX = "BNB_ECONOMIC_FIRST_B28M_LONG_12_13WIB_CHARACTER"
...
def main():
    ...
    lines = [
        "# BNB Economic-First B28M — 12:00–13:00 WIB LONG Character Discovery Result", "",
        f"Raw BNBUSDT 5m coverage: **{coverage:.4%}**.",
        "Direction: **LONG only**.",
        "Time habitat: **12:00–13:00 WIB only** (05:00, 05:15, 05:30, 05:45 UTC anchors).",
        "Methodology and gates: **identical to preregistered B28A–B28L; only the hour changed**.",
    ...
    if len(C) == 0:
        status = "BNB_ECONOMIC_FIRST_B28M_NO_LONG_CHARACTER"
        OUT_STATUS.write_text(status + "\n")
        lines += ["", f"**Status: {status}**", "",
                  "No 12:00–13:00 WIB BNB LONG character passed the preregistered anchor-stability + pooled-economics/risk + all-era gates.",
                  "No gate relaxation, prior-hour rescue, B27 structural rescue, or OOS exposure is allowed.",
                  "Next preregistered habitat: **13:00–14:00 WIB** with the unchanged methodology.", "", "Research/shadow only."]
    ...
    status = "BNB_ECONOMIC_FIRST_B28M_LONG_CHARACTER_FOUND"
    OUT_STATUS.write_text(status + "\n")
