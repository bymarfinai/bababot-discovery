...
Implementation note: indexed 5m/1H lookups replace repeated full-dataframe scans.
This changes runtime only, not the frozen SR81 research definition.
"""
...
LOOKBACK=pd.Timedelta(days=7);EPISODE=pd.Timedelta(hours=6);APPROACH_ATR=.10


def wilson(w:int,n:int):
    if n<=0:return [None,None]
    p=w/n;z=1.959963984540054;den=1+z*z/n
    c=(p+z*z/(2*n))/den;hh=z*math.sqrt((p*(1-p)+z*z/(4*n))/n)/den
...
def touch_atr(h:pd.DataFrame,t:pd.Timestamp):
    # Exact equivalent of completed_h1_before(h,t).iloc[-1], via indexed lookup.
    cutoff=t-pd.Timedelta(hours=1)
    j=int(h.index.searchsorted(cutoff,side='right'))-1
    if j<0:return np.nan
    return float(h.iloc[j].atr14)

def resolve_fast(k:pd.DataFrame,touch:pd.Timestamp,level:float,side:str,atr:float):
    """Exact SR80 0.5 ATR / 6h ambiguity semantics using positional lookup."""
    j=int(k.index.searchsorted(touch,side='left'))
    if j>=len(k) or k.index[j]!=touch:return {'outcome':'INTEGRITY_ERROR'}
    up=level+sr.REACTION_ATR*atr;dn=level-sr.REACTION_ATR*atr
    b0=k.iloc[j]
    if float(b0.high)>=up or float(b0.low)<=dn:return {'outcome':'AMBIGUOUS_TOUCH_BAR'}
    end=touch+EPISODE;i=j+1
...
def prior_proof(k:pd.DataFrame,h:pd.DataFrame,fs:pd.Timestamp,level:float,side:str):
    ...
    idx=hist.index;i=1;events=[]
    while i<len(hist):
        t=idx[i];b=hist.iloc[i];prev=hist.iloc[i-1];atr=touch_atr(h,t)
        if not np.isfinite(atr) or atr<=0:i+=1;continue
        touched=float(b.low)<=level<=float(b.high)
        eligible=touched and (float(prev.close)>level+APPROACH_ATR*atr if side=='SUPPORT' else float(prev.close)<level-APPROACH_ATR*atr)
        if not eligible:i+=1;continue
        r=resolve_fast(k,t,level,side,atr);events.append({'touch':str(t),'atr':atr,'outcome':r['outcome']})
        resume=t+EPISODE
        i=int(idx.searchsorted(resume,side='left'))
    outcomes=[e['outcome'] for e in events]
    return {'resolved':sum(o in {'HOLD','BREAK'} for o in outcomes),'hold':sum(o=='HOLD' for o in outcomes),
            'break':sum(o=='BREAK' for o in outcomes),'ambiguous':sum(str(o).startswith('AMBIGUOUS') for o in outcomes),
...
def build(k:pd.DataFrame,h:pd.DataFrame):
    ...
    for fs in sr.friday_dates():
        ...
        for ci,c in enumerate(levels):
            ...
            mask=(friday.low.to_numpy(float)<=level)&(friday.high.to_numpy(float)>=level)
            hits=np.flatnonzero(mask)
            if len(hits)==0:continue
            touched_any=True;proof=prior_proof(k,h,fs,level,side)
            proven=proof['resolved']>=2 and proof['hold']==proof['resolved'] and proof['break']==0
            if not proven:continue
            touch=friday.index[int(hits[0])];out=resolve_fast(k,touch,level,side,atr)
            if any(pd.Timestamp(o)>=fs for o in c['origins']):viol+=1
            rows.append({'friday_wib':str((fs+pd.Timedelta(hours=7)).date()),'freeze_utc':str(fs),'touch_utc':str(touch),
              ...
              'prior_ambiguous':proof['ambiguous'],'prior_unresolved':proof['unresolved'],'outcome':out['outcome']})
...
def blocks(z:pd.DataFrame):
    ...
    for i,ch in enumerate(np.array_split(np.array(dates,dtype=object),4)):
        out[f'B{i+1}']=stats(z[z.friday_wib.isin(set(ch))])
    return out

def main():
    k=fast.fast_load();h=sr.build_h1(k);events,base_dates,viol=build(k,h)
    if events.empty:
    ...
    sd,sv,sf=stats(d),stats(v),stats(full);bl=blocks(full);pos=sum(q['n']>=5 and q['rate'] is not None and q['rate']>.50 for q in bl.values())
    ...
    sides={s:stats(full[full.side==s]) for s in ['SUPPORT','RESISTANCE']};fam={f:stats(full[full.families.astype(str).str.contains(f,regex=False)]) for f in ['PDAY','W7','SWING']}
    ...
    out={'protocol':'SR81','rule':'prior resolved same-side events >=2; all resolved prior events HOLD; zero BREAK',
         'base_friday_dates_with_any_level_touch':len(base_dates),'prior_proven_touch_events':len(events),'outcome_counts':{str(k):int(vv) for k,vv in counts.items()},
         'resolved_events':len(full),'discovery':sd,'validation':sv,'full':sf,'blocks':bl,'positive_blocks':pos,
