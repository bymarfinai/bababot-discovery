...
PFX = "BNB_ECONOMIC_FIRST_B28H_LONG_07_08WIB_CHARACTER"
...
def main():
    ...
    lines = [
        "# BNB Economic-First B28H — 07:00–08:00 WIB LONG Character Discovery Result", "",
        f"Raw BNBUSDT 5m coverage: **{coverage:.4%}**.",
        "Direction: **LONG only**.",
        "Time habitat: **07:00–08:00 WIB only** (00:00, 00:15, 00:30, 00:45 UTC anchors).",
        "Methodology and gates: **identical to preregistered B28A–B28G; only the hour changed**.",
    ...
    if len(C) == 0:
        status = "BNB_ECONOMIC_FIRST_B28H_NO_LONG_CHARACTER"
        OUT_STATUS.write_text(status + "\n")
        lines += ["", f"**Status: {status}**", "",
                  "No 07:00–08:00 WIB BNB LONG character passed the preregistered anchor-stability + pooled-economics/risk + all-era gates.",
                  "No gate relaxation, prior-hour rescue, B27 structural rescue, or OOS exposure is allowed.",
                  "Next preregistered habitat: **08:00–09:00 WIB** with the unchanged methodology.", "", "Research/shadow only."]
    ...
    status = "BNB_ECONOMIC_FIRST_B28H_LONG_CHARACTER_FOUND"
    OUT_STATUS.write_text(status + "\n")
