...
PFX = 'BNB_HOURLY_STRUCTURE_B27FB'
...
def build_sessions(x5: pd.DataFrame) -> pd.DataFrame:
    ...
    for d in pd.date_range(local_first, local_last, freq='D'):
        ...
        if len(ref) != 48 or len(exe) != 48:
            raise AssertionError(
                f'incomplete B27FB session {day}: ref={len(ref)}/48 exe={len(exe)}/48'
            )
    ...
    d = pd.DataFrame(rows)
    if d.empty:
        raise AssertionError('no B27FB sessions')
    return d
...
def main():
    ...
    if not prereg.exists():
        raise AssertionError('B27FB preregistration missing')
    ...
    b27fa_leaves = 137
    b27fa_h2 = 105
    b27fa_h2_rate = 105 / 137
    delta_pp = 100.0 * (pooled['h2_rate'] - b27fa_h2_rate) if not pd.isna(pooled['h2_rate']) else np.nan

    lines = [
        '# BNB Hour-by-Hour Structural Discovery — B27FB',
        '',
        '**Anchor tested:** 01:00 WIB only.',
        '',
        '- Reference: 01:00–05:00 WIB',
        '- Execution: 05:00–09:00 WIB',
        '- Development only: 2022-01-01 through 2025-01-01 UTC',
        f'- Raw loader coverage: {coverage:.4%}',
        '- Structure only: K1 -> causal leave -> H2 using frozen B27EM/B27FA causal ordering',
        ...
        '',
        '## Frozen comparison vs B27FA / 00:00 WIB',
        '',
        f'- B27FA leaves/H2: **{b27fa_leaves}/{b27fa_h2}**, H2/leave **{100*b27fa_h2_rate:.1f}%**',
        f"- B27FB leaves/H2: **{pooled['causal_leave']}/{pooled['h2']}**, H2/leave **{pct(pooled['h2_rate'])}**",
        f'- B27FB minus B27FA H2-rate delta: **{delta_pp:+.1f} percentage points**',
        '',
        '## Weekday breakdown',
        '',
    ...
    for _, r in s[s.scope != 'POOLED'].iterrows():
        ...
        lines.append(
            f"| {r.scope} | {int(r.sessions)} | {int(r.k1_qualified)} | {int(r.causal_leave)} | {int(r.h2)} | {pct(r.h2_rate)} | {int(r.opposite_break_before_h2)} | {int(r.no_h2_by_end)} | {pct(r.resolved_h2_share)} | {med} |"
        )
    ...
    lines += [
        ...
        '## Interpretation',
        '',
        'This milestone ranks only whether the 01:00 WIB clock geometry produces a repeatable LONG revisit structure under the same state machine as 00:00 WIB. The H2 rate is not trading WR and cannot be compared to TP/SL WR.',
        '',
        '**Status: B27FB_BNB_HOUR01_STRUCTURE_COMPLETE**',
        '',
        'STOP: do not test 02:00 WIB or define an entry in B27FB.',
    ]

    OUT_MD.write_text('\n'.join(lines) + '\n', encoding='utf-8')
    OUT_STATUS.write_text('B27FB_BNB_HOUR01_STRUCTURE_COMPLETE\n', encoding='utf-8')
    print('\n'.join(lines))
