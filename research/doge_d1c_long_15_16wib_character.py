...
# D1C changes only the fixed one-hour clock habitat.
import eth_economic_first_e12a_long_13_14wib_character as engine

ROOT = Path(__file__).resolve().parent.parent
PFX = "DOGE_D1C_LONG_15_16WIB_CHARACTER"
...
# Frozen D1C habitat only: 08:00..08:45 UTC = 15:00..15:45 WIB.
engine.CLOCKS = (480, 495, 510, 525)
...
def gate_failure(r):
    ...
    if not bool(r.pooled_gate):
        reasons = []
        if int(r.trades) < 160: reasons.append("N")
        if not np.isfinite(r.win_rate) or float(r.win_rate) < .55: reasons.append("WR")
        if float(r.net_pnl) <= 0: reasons.append("net")
        if not np.isfinite(r.expectancy) or float(r.expectancy) < .50: reasons.append("expectancy")
        if not np.isfinite(r.pf) or float(r.pf) < 1.20: reasons.append("PF")
        if not np.isfinite(r.max_dd) or float(r.max_dd) > 125: reasons.append("DD")
        if int(r.max_loss_streak) > 8: reasons.append("loss streak")
        failures.append("pooled(" + ", ".join(reasons) + ")")
...
def main():
    ...
    lines = [
        "# DOGE D1C — 15:00–16:00 WIB LONG Character Discovery Result", "",
        f"Raw DOGEUSDT 5m coverage: **{coverage:.4%}**.",
        "Direction: **LONG only**.",
        "Time habitat: **15:00–16:00 WIB only** (08:00, 08:15, 08:30, 08:45 UTC anchors).",
    ...
    if len(C) == 0:
        status = "DOGE_D1C_NO_LONG_CHARACTER"
        best = rank.iloc[0]
        OUT_STATUS.write_text(status + "\n")
        lines += [
            "", "## Formal verdict", "", f"**Status: {status}**", "",
            "No 15:00–16:00 WIB DOGE LONG candidate passed anchor + pooled + all-era gates simultaneously.", "",
    ...
    status = "DOGE_D1C_LONG_CHARACTER_FOUND"
    OUT_STATUS.write_text(status + "\n")
    lines += [
