...
PFX = 'BNB_HOURLY_STRUCTURE_B27FC'
...
def build_sessions(x5: pd.DataFrame) -> pd.DataFrame:
    ...
    for d in pd.date_range(local_first, local_last, freq='D'):
        ...
        if len(ref) != 48 or len(exe) != 48:
            raise AssertionError(f'incomplete B27FC session {day}: ref={len(ref)}/48 exe={len(exe)}/48')
    ...
    d = pd.DataFrame(rows)
    if d.empty:
        raise AssertionError('no B27FC sessions')
    return d
...
def main():
    ...
    if not prereg.exists():
        raise AssertionError('B27FC preregistration missing')
    ...
    leader_slot, leader_leaves, leader_h2, leader_rate = max(prior, key=lambda x: x[3])
    ...
    lines = [
        ...
        f"- 02:00 WIB: **{pooled['causal_leave']} leaves / {pooled['h2']} H2 = {pct(pooled['h2_rate'])}**",
        f'- B27FC minus prior leader ({leader_slot}) H2-rate delta: **{delta_pp:+.1f} percentage points**','',
        '## Weekday breakdown','',
    ...
    OUT_MD.write_text('\n'.join(lines) + '\n', encoding='utf-8')
    OUT_STATUS.write_text('B27FC_BNB_HOUR02_STRUCTURE_COMPLETE\n', encoding='utf-8')
    print('\n'.join(lines))
