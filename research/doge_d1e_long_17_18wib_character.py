...
# pair-agnostic E12 engine. D1E changes only the clock habitat and labels.
import doge_d1d_long_16_17wib_character as frozen

engine = frozen.engine
ROOT = Path(__file__).resolve().parent.parent
PFX = "DOGE_D1E_LONG_17_18WIB_CHARACTER"
...
# Frozen D1E habitat only: 10:00..10:45 UTC = 17:00..17:45 WIB.
engine.CLOCKS = (600, 615, 630, 645)
...
def main():
    ...
    lines = [
        "# DOGE D1E — 17:00–18:00 WIB LONG Character Discovery Result", "",
    ...
    if len(C) == 0:
        status = "DOGE_D1E_NO_LONG_CHARACTER"
        best = rank.iloc[0]
        OUT_STATUS.write_text(status + "\n")
        lines += [
            "", "## Formal verdict", "", f"**Status: {status}**", "",
            "No 17:00–18:00 WIB DOGE LONG candidate passed anchor + pooled + all-era gates simultaneously.", "",
    ...
    status = "DOGE_D1E_LONG_CHARACTER_FOUND"
    OUT_STATUS.write_text(status + "\n")
    lines += [
