...
# pair-agnostic E12 engine. D1I changes only the clock habitat and labels.
import doge_d1d_long_16_17wib_character as frozen

engine = frozen.engine
ROOT = Path(__file__).resolve().parent.parent
PFX = "DOGE_D1I_LONG_21_22WIB_CHARACTER"
...
# Frozen D1I habitat only: 14:00..14:45 UTC = 21:00..21:45 WIB.
...
def main():
    ...
    lines = [
        "# DOGE D1I — 21:00–22:00 WIB LONG Character Discovery Result", "",
    ...
    if len(C) == 0:
        status = "DOGE_D1I_NO_LONG_CHARACTER"
        best = rank.iloc[0]
        OUT_STATUS.write_text(status + "\n")
        lines += [
            "", "## Formal verdict", "", f"**Status: {status}**", "",
            "No 21:00–22:00 WIB DOGE LONG candidate passed anchor + pooled + all-era gates simultaneously.", "",
    ...
    status = "DOGE_D1I_LONG_CHARACTER_FOUND"
    OUT_STATUS.write_text(status + "\n")
    lines += [
