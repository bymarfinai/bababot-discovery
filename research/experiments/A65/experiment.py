...
EPS = 1e-12
DIR = {
    "recovery_from_worst_R": "lower",
    "recovery_efficiency": "lower",
    "bars_since_worst_fraction": "lower",
    "post_worst_close_slope_R_per_bar": "lower",
}


def load_a55(root: Path):
    p = root / "research" / "sol_long_15utc_loss_confirmation_candle_a55.py"
    s = importlib.util.spec_from_file_location("a65_a55", p)
...
def path_features(market, r, age):
    idx = market["idx"]
    ...
    mae = max(0.0, (H - worst_low) / R)
    recovery = (float(cl[-1]) - worst_low) / R
    recovery_efficiency = recovery / mae if mae > EPS else np.nan
    bars_since_fraction = float((len(lo) - 1 - worst_i) / max(1, len(lo) - 1))
    post = cl[worst_i:]
    if len(post) <= 1:
        slope = 0.0
    else:
        x = np.arange(len(post), dtype=float)
        slope = float(np.polyfit(x, post, 1)[0] / R)
    f = {
        "running_mae_R": mae,
        "recovery_from_worst_R": recovery,
        "recovery_efficiency": recovery_efficiency,
        "bars_since_worst_fraction": bars_since_fraction,
        "post_worst_close_slope_R_per_bar": slope,
    }
    for k, v in f.items():
        if k == "recovery_efficiency" and pd.isna(v):
            continue
        if not np.isfinite(v):
            raise RuntimeError(f"nonfinite {k} at {r.entry_ts} age={age}")
    return s, t, f
...
def choose_control(pool, case, case_f, age, market):
    ...
    for _, w in pool.iterrows():
        ...
        if not eligible_winner(w, t):
            continue
        _, _, wf = path_features(market, w, age)
        ...
        candidates.append(
            (
                mae_gap,
                calendar_gap,
                pd.Timestamp(w.execution_start),
                pd.Timestamp(w.entry_ts),
                w,
                wf,
            )
        )
...
def run(context):
        ...
        for age in AGES:
            ...
            for _, r in cases.iterrows():
                ...
                if not eligible_l0(r, t):
                    continue
                eligible += 1
                cs, ct, cf = path_features(market, r, age)
                chosen = choose_control(controls, r, cf, age, market)
                if chosen is None:
                    continue
                w, wf, mae_gap = chosen
                ws, wt, _ = path_features(market, w, age)
                matched += 1
                ...
                z = {
                    ...
                    "control_last_completed_bar_ts": wt,
                    "control_pnl": float(w.pnl),
                    "case__running_mae_R": cf["running_mae_R"],
                    "control__running_mae_R": wf["running_mae_R"],
    ...
    if events.empty or events.duplicated(["partition", "snapshot_age_min", "case_entry_ts"]).any():
        raise RuntimeError("empty or duplicate A65 events")
    ...
    status = (
        "SOL_LONG_15UTC_L0_MAE_CONDITIONED_RECOVERY_A65_SUPPORTED"
        if supported
        else "SOL_LONG_15UTC_L0_MAE_CONDITIONED_RECOVERY_A65_INCONCLUSIVE"
    )
    ...
    artifacts = []
    prefix = "SOL_LONG_15UTC_L0_MAE_CONDITIONED_RECOVERY_A65"
    ...
    for _, r in coverage.iterrows():
        lines.append(
            f"| {SHOW[r.partition]} | {int(r.snapshot_age_min)}m | {int(r.eligible_l0_n)} | {int(r.matched_n)} | {int(r.unique_control_n)} | {int(r.max_control_reuse)}x | {fmt(r.median_abs_mae_gap)}R | {fmt(r.p75_abs_mae_gap)}R | {fmt(r.max_abs_mae_gap)}R |"
        )
    lines += [
        "",
    ...
    lines += [
        "",
        f"Supported recovery feature families: **{', '.join(supported) if supported else 'none'}**.",
        "",
    ...
    summary = {
        ...
        "supported_recovery_feature_families": supported,
        ...
        "median_abs_mae_match_gap_by_partition_snapshot": {
            f"{r.partition}:{int(r.snapshot_age_min)}m": float(r.median_abs_mae_gap)
            for _, r in coverage.iterrows()
        },
        "interpretation_boundary": "mae_conditioned_recovery_anatomy_only_no_intervention",
    }
    return {
        "observed": observed,
        "summary": summary,
