...
PFX = "BNB_B28I_FROZEN_OOS_VALIDATION"
...
CLOCKS = (60, 75, 90, 105)  # 01:00..01:45 UTC = 08:00..08:45 WIB
LB = 360
HOLD = 720
...
def anchor_atlas(T):
    ...
    for clock in CLOCKS:
        ...
        rows.append({
            "clock_utc": engine.e11.hhmm(clock), "clock_wib": engine.e11.wib(clock),
...
def main():
    ...
    for p in PRIMARY_PARTS:
        metric_rows.append({"scope": p, **part_s[p]})
    metric_rows.append({"scope": "POOLED_PRIMARY_OOS", **pooled_s})
    metric_rows.append({"scope": "AUGUST_SHADOW", **shadow_s})
    pd.DataFrame(metric_rows).to_csv(OUT_METRICS, index=False)

    if not data_gate:
        status = "BNB_B28I_OOS_DATA_TOOLING_FAILURE"
    elif all(p_gates.values()) and pooled_ok and anchor_ok:
        status = "BNB_B28I_OOS_PASS"
    else:
        status = "BNB_B28I_OOS_FUNDAMENTAL_REJECT"
    OUT_STATUS.write_text(status + "\n")

    lines = [
        "# BNB B28I — Frozen OOS Validation Result", "",
        f"**Status: {status}**", "",
        "## Frozen identity", "",
        f"- BNBUSDT LONG, 08:00–09:00 WIB",
    ...
    lines += ["", f"Anchor gate: **{'PASS' if anchor_ok else 'FAIL'}** ({support_n}/{eval_n} supportive/evaluable).", "",
              ...
              "## Frozen decision", "",
              f"- External partition gate: **{'PASS' if p_gates['external'] else 'FAIL'}**",
              ...
              f"- Anchor-stability gate: **{'PASS' if anchor_ok else 'FAIL'}**",
              f"- Final: **{status}**", "", "No live orders were placed. Research/shadow validation only."]
