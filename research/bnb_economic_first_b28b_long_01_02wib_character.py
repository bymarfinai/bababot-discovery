...
PFX = "BNB_ECONOMIC_FIRST_B28B_LONG_01_02WIB_CHARACTER"
...
# 18:00..18:45 UTC = 01:00..01:45 WIB.
...
def main():
    ...
    lines = [
        "# BNB Economic-First B28B — 01:00–02:00 WIB LONG Character Discovery Result", "",
        f"Raw BNBUSDT 5m coverage: **{coverage:.4%}**.",
        "Direction: **LONG only**.",
        "Time habitat: **01:00–02:00 WIB only** (18:00, 18:15, 18:30, 18:45 UTC anchors).",
        "Methodology and gates: **identical to preregistered B28A; only the hour changed**.",
        "B28A winner and all B27 H2/leave/P10 coordinates receive no candidate-ranking preference.",
    ...
    if len(C) == 0:
        status = "BNB_ECONOMIC_FIRST_B28B_NO_LONG_CHARACTER"
        OUT_STATUS.write_text(status + "\n")
        lines += ["", f"**Status: {status}**", "",
                  "No 01:00–02:00 WIB BNB LONG character passed the preregistered anchor-stability + pooled-economics + all-era gates.",
                  "No gate relaxation, prior-hour preference, B27 structural rescue, or OOS exposure.",
                  "Next preregistered habitat: **02:00–03:00 WIB** with unchanged methodology.", "", "Research/shadow only."]
    ...
    status = "BNB_ECONOMIC_FIRST_B28B_LONG_CHARACTER_FOUND"
    OUT_STATUS.write_text(status + "\n")
    ...
    lines += ["", f"**Status: {status}**", "",
              "Prior-hour/B27 structure remained sealed from selection. OOS stays closed.", "", "Research/shadow only."]
    OUT_RESULT.write_text("\n".join(lines) + "\n")
    print(OUT_RESULT.read_text())
