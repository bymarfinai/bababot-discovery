...
def attach_range_completion(c: pd.DataFrame, x5: pd.DataFrame):
    ...
    for r in q.itertuples(index=False):
        ...
        parity.append({'partition': r.partition, 'zone': r.zone, 'reference_start': r.reference_start, 'check': 'H', 'actual': H, 'expected': r.H, 'pass': hp})
        parity.append({'partition': r.partition, 'zone': r.zone, 'reference_start': r.reference_start, 'check': 'L', 'actual': L, 'expected': r.L, 'pass': lp})
        if not (hp and lp):
