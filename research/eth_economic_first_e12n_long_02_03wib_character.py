...
PFX = "ETH_ECONOMIC_FIRST_E12N_LONG_02_03WIB_CHARACTER"
...
def main():
    ...
    lines = [
        "# ETH Economic-First E12N — 02:00–03:00 WIB LONG Character Discovery Result", "",
        f"Raw ETHUSDT 5m coverage: **{coverage:.4%}**.",
        "Direction: **LONG only**.",
        "Time habitat: **02:00–03:00 WIB only** (19:00, 19:15, 19:30, 19:45 UTC anchors).",
        "Scientific grammar and gates: **identical to preregistered E12A–E12M; only the hour changed**.",
    ...
    if len(C) == 0:
        status = "ETH_ECONOMIC_FIRST_E12N_NO_LONG_CHARACTER"
        OUT_STATUS.write_text(status + "\n")
        lines += ["", f"**Status: {status}**", "", "No 02:00–03:00 WIB LONG character passed the preregistered anchor-stability + pooled-economics + all-era gates.", "No gate relaxation and no OOS exposure.", "", "Research/shadow only."]
    ...
    status = "ETH_ECONOMIC_FIRST_E12N_LONG_CHARACTER_FOUND"
    OUT_STATUS.write_text(status + "\n")
