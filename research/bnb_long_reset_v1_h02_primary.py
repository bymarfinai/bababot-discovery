...
# H02 is a habitat-only shift. All Stage-2 methodology, features, bins,
# gates, ranking, Development years, horizons, and causal rules remain frozen.
TARGET_LOCAL_HOUR = 2
PFX = "BNB_LONG_RESET_V1_H02_PRIMARY"
base.PFX = PFX
...
def build_events_h02(x: pd.DataFrame) -> pd.DataFrame:
    rows = []
    idx = x.index
...
base.build_events = build_events_h02
...
def main() -> None:
    ...
    if base.OUT_RESULT.exists():
        ...
        text = text.replace("H00", "H02")
        text = text.replace("00:00-01:00 WIB", "02:00-03:00 WIB")
        text = text.replace("00:00–01:00 WIB", "02:00–03:00 WIB")
