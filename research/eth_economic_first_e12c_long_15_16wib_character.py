...
PFX = "ETH_ECONOMIC_FIRST_E12C_LONG_15_16WIB_CHARACTER"
...
CLOCKS = (480, 495, 510, 525)  # 08:00..08:45 UTC = 15:00..15:45 WIB
...
def main():
    # Reuse the preregistered E12A/E12B scientific grammar and gates; change only the one-hour habitat.
    e12a.CLOCKS = CLOCKS
    ...
    lines = [
        "# ETH Economic-First E12C — 15:00–16:00 WIB LONG Character Discovery Result", "",
        f"Raw ETHUSDT 5m coverage: **{coverage:.4%}**.",
        "Direction: **LONG only**.",
        "Time habitat: **15:00–16:00 WIB only** (08:00, 08:15, 08:30, 08:45 UTC anchors).",
        "Scientific grammar and gates: **identical to preregistered E12A/E12B; only the hour changed**.",
    ...
    if len(C) == 0:
        status = "ETH_ECONOMIC_FIRST_E12C_NO_LONG_CHARACTER"
        OUT_STATUS.write_text(status + "\n")
        lines += ["", f"**Status: {status}**", "",
                  "No 15:00–16:00 WIB LONG character passed the preregistered anchor-stability + pooled-economics + all-era gates.",
    ...
    status = "ETH_ECONOMIC_FIRST_E12C_LONG_CHARACTER_FOUND"
    OUT_STATUS.write_text(status + "\n")
    ...
    lines += ["", f"**Status: {status}**", "",
              "Discovery-only: OOS stays closed. Freeze this one character for the next bounded follow-up; do not reopen the E12C search grid.", "", "Research/shadow only."]
    OUT_RESULT.write_text("\n".join(lines) + "\n")
    print(OUT_RESULT.read_text())
