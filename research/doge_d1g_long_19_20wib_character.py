...
# pair-agnostic E12 engine. D1G changes only the clock habitat and labels.
import doge_d1d_long_16_17wib_character as frozen

engine = frozen.engine
ROOT = Path(__file__).resolve().parent.parent
PFX = "DOGE_D1G_LONG_19_20WIB_CHARACTER"
...
# Frozen D1G habitat only: 12:00..12:45 UTC = 19:00..19:45 WIB.
engine.CLOCKS = (720, 735, 750, 765)
...
def main():
    ...
    lines = [
        "# DOGE D1G — 19:00–20:00 WIB LONG Character Discovery Result", "",
    ...
    if len(C) == 0:
        status = "DOGE_D1G_NO_LONG_CHARACTER"
        best = rank.iloc[0]
        OUT_STATUS.write_text(status + "\n")
        lines += [
            "", "## Formal verdict", "", f"**Status: {status}**", "",
            "No 19:00–20:00 WIB DOGE LONG candidate passed anchor + pooled + all-era gates simultaneously.", "",
    ...
    status = "DOGE_D1G_LONG_CHARACTER_FOUND"
    OUT_STATUS.write_text(status + "\n")
    lines += [
