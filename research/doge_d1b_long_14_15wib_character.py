...
# D1B changes only the fixed one-hour clock habitat.
import eth_economic_first_e12a_long_13_14wib_character as engine

ROOT = Path(__file__).resolve().parent.parent
PFX = "DOGE_D1B_LONG_14_15WIB_CHARACTER"
...
# Frozen D1B habitat only: 07:00..07:45 UTC = 14:00..14:45 WIB.
engine.CLOCKS = (420, 435, 450, 465)
...
def main():
    ...
    lines = [
        "# DOGE D1B — 14:00–15:00 WIB LONG Character Discovery Result", "",
        f"Raw DOGEUSDT 5m coverage: **{coverage:.4%}**.",
        "Direction: **LONG only**.",
        "Time habitat: **14:00–15:00 WIB only** (07:00, 07:15, 07:30, 07:45 UTC anchors).",
    ...
    if len(C) == 0:
        status = "DOGE_D1B_NO_LONG_CHARACTER"
        best = rank.iloc[0]
        OUT_STATUS.write_text(status + "\n")
        lines += [
            "", "## Formal verdict", "", f"**Status: {status}**", "",
            "No 14:00–15:00 WIB DOGE LONG candidate passed anchor + pooled + all-era gates simultaneously.", "",
    ...
    status = "DOGE_D1B_LONG_CHARACTER_FOUND"
    OUT_STATUS.write_text(status + "\n")
    lines += [
