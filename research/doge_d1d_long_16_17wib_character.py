...
# D1D changes only the fixed one-hour clock habitat.
import eth_economic_first_e12a_long_13_14wib_character as engine

ROOT = Path(__file__).resolve().parent.parent
PFX = "DOGE_D1D_LONG_16_17WIB_CHARACTER"
...
# Frozen D1D habitat only: 09:00..09:45 UTC = 16:00..16:45 WIB.
engine.CLOCKS = (540, 555, 570, 585)
...
def main():
    ...
    lines = [
        "# DOGE D1D — 16:00–17:00 WIB LONG Character Discovery Result", "",
        f"Raw DOGEUSDT 5m coverage: **{coverage:.4%}**.",
        "Direction: **LONG only**.",
        "Time habitat: **16:00–17:00 WIB only** (09:00, 09:15, 09:30, 09:45 UTC anchors).",
    ...
    if len(C) == 0:
        status = "DOGE_D1D_NO_LONG_CHARACTER"
        best = rank.iloc[0]
        OUT_STATUS.write_text(status + "\n")
        lines += [
            "", "## Formal verdict", "", f"**Status: {status}**", "",
            "No 16:00–17:00 WIB DOGE LONG candidate passed anchor + pooled + all-era gates simultaneously.", "",
    ...
    status = "DOGE_D1D_LONG_CHARACTER_FOUND"
    OUT_STATUS.write_text(status + "\n")
    lines += [
