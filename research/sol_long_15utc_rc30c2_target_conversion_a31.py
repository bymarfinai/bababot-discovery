...
OUT_MD=ROOT/"SOL_LONG_15UTC_RC30C2_TARGET_CONVERSION_A31_Result.md"
OUT_DEV=ROOT/"SOL_LONG_15UTC_RC30C2_TARGET_CONVERSION_A31_DEVELOPMENT.csv"
OUT_OOS=ROOT/"SOL_LONG_15UTC_RC30C2_TARGET_CONVERSION_A31_OOS.csv"
OUT_TRADES=ROOT/"SOL_LONG_15UTC_RC30C2_TARGET_CONVERSION_A31_TRADES.csv"
OUT_STATUS=ROOT/"SOL_LONG_15UTC_RC30C2_TARGET_CONVERSION_A31_Status.txt"

LANES={"T10_FULL":.10,"T20_FULL":.20}
CELLS=a27.CELLS; STRESS=a2.STRESS
...
def simulate_one(m,r,lane):
    target_R=LANES[lane]; w=a4.recovery_window(m,r)
    if w is None:return None
    _,xi,endpos,_=w; idx=m["idx"]; op=m["open"]; hi=m["high"]; cl=m["close"]
    H=float(r.H); L=float(r.L); R=float(r.R); target=H+target_R*R
    search_end=min(xi+30//5,endpos); count=0; signal=-1
    # exact RC30_C2 signal. Only E40 pre-signal cancels as in A27; lower candidate targets are not credited before entry.
    e40=H+.40*R
    ...
    # target not credited on signal or entry bar
    ...
    return {"role":r.role,"partition":r.partition,"dev_block":r.dev_block,"execution_start":r.execution_start,"lane":lane,"target_R":target_R,
            "H":H,"L":L,"R":R,"parent_entry_ts":r.entry_ts,"parent_exit_ts":r.exit_ts,"parent_pnl":float(r.pnl),"parent_pnl_5bps":float(r.pnl_5bps),
            "signal_ts":idx[signal],"signal_delay_min":float((signal-xi+1)*5),"reentry_ts":idx[entry_i],"reentry_price":entry,
            "exit_ts":idx[exit_i],"exit_price":exit_price,"exit_reason":reason,"invalidation_close_ts":idx[invalid] if invalid>=0 else pd.NaT,
...
def stats(parent,t):
    ...
    return {"parent_n":len(parent),"parent_wr":float((bp>0).mean()),"parent_pf":pf(bp),"parent_net":float(bp.sum()),"parent_wr_5bps":float((bp5>0).mean()),"parent_pf_5bps":pf(bp5),"parent_net_5bps":float(bp5.sum()),
            ...
            "overlay_net_improvement":float(r.sum()),"overlay_net_improvement_5bps":float(r5.sum()),"target_hit_rate":float((t.exit_reason.astype(str)=="TARGET").mean()) if len(t) else np.nan}
...
def dev_row(parent,t,lane):
    ...
    uplift=s["episode_wr"]-s["parent_wr"]
    eligible=bool(s["recovery_n"]>=60 and s["recovery_wr"]>=.45 and s["recovery_pf"]>1.20 and s["recovery_pf_5bps"]>1.05 and s["recovery_exp"]>0 and s["recovery_exp_5bps"]>0 and s["recovery_net"]>0 and s["recovery_net_5bps"]>0 and s["overlay_pf"]>s["parent_pf"] and s["overlay_pf_5bps"]>s["parent_pf_5bps"] and s["overlay_net"]>s["parent_net"] and s["overlay_net_5bps"]>s["parent_net_5bps"] and uplift>=.05 and s["rescue_rate"]>=.35 and adequate>=4 and pos>=4 and pos5>=4)
    return {"lane":lane,"target_R":LANES[lane],**s,"episode_wr_uplift":uplift,"adequate_blocks":adequate,"positive_blocks_raw":pos,"positive_blocks_5bps":pos5,"eligible":eligible,**blocks}

def choose(dev):
    q=dev[dev.eligible.astype(bool)].copy()
    if q.empty:return None
    return q.sort_values(["overlay_net_improvement_5bps","overlay_pf_5bps","episode_wr_uplift","target_R"],ascending=[False,False,False,False]).iloc[0]
...
def main():
    ...
    if winner is None:status="SOL_LONG_15UTC_RC30C2_TARGET_CONVERSION_A31_REJECTED_DEVELOPMENT"
        ...
        status="SOL_LONG_15UTC_RC30C2_TARGET_CONVERSION_A31_SUPPORTED" if supported else "SOL_LONG_15UTC_RC30C2_TARGET_CONVERSION_A31_REJECTED_OOS"
    (pd.concat(alltr,ignore_index=True) if alltr else pd.DataFrame()).to_csv(OUT_TRADES,index=False)
    lines=["# SOL LONG 15:00 UTC RC30_C2 Target Conversion — A31 Result","",f"Raw SOLUSDT 5m coverage: **{100*coverage:.4f}%**.","",
           "A31 tests only the A30-authorized E10 and E20 full recovery targets on the exact RC30_C2 signal.","","## Development","",
           "| Lane | N | Target hit | Rec WR | Rec PF | Rec Net | 5bps PF | 5bps Net | Rescue | Parent WR→Episode WR | PF→Overlay | 5bps PF→Overlay | +blocks raw/stress | Pass |","|---|---:|---:|---:|---:|---:|---:|---:|---:|---|---|---|---|---|"]
    for _,r in dev.iterrows():
        lines.append(f"| {r.lane} | {int(r.recovery_n)} | {pct(r.target_hit_rate)} | {pct(r.recovery_wr)} | {fmt(r.recovery_pf)} | ${fmt(r.recovery_net)} | {fmt(r.recovery_pf_5bps)} | ${fmt(r.recovery_net_5bps)} | {pct(r.rescue_rate)} | {pct(r.parent_wr)}→{pct(r.episode_wr)} | {fmt(r.parent_pf)}→{fmt(r.overlay_pf)} | {fmt(r.parent_pf_5bps)}→{fmt(r.overlay_pf_5bps)} | {int(r.positive_blocks_raw)}/{int(r.positive_blocks_5bps)} | {'YES' if bool(r.eligible) else 'NO'} |")
    ...
    lines += ["","## Decision","",f"**Status: {status}**","","No neighboring target scan is allowed after A31.","","Research only. Live Baba Bot remains unchanged."]
