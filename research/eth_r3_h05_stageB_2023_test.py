...
LOCK_PATH = ROOT / "ETH_R3_H05_STAGEA_FrozenDevCandidate.csv"
OUT_NEIGHBOR = ROOT / "ETH_R3_H05_STAGEB_2023_LocalNeighborhood.csv"
OUT_LOCK = ROOT / "ETH_R3_H05_STAGEB_FrozenStableCandidate.csv"
OUT_RESULT = ROOT / "ETH_R3_H05_STAGEB_Result.md"
OUT_STATUS = ROOT / "ETH_R3_H05_STAGEB_Status.txt"
...
def main():
    ...
    for lb, hold, dist in coords:
        ...
        rows.append({
            ...
            "dd": dd, "ls": int(s["max_loss_streak"]), "wr_change_pp": wr_change_pp,
            "exp_retention": exp_ret, "pf_retention": pf_ret,
            "dd_cap": dd_cap, "ls_warning_threshold": ls_warning,
    ...
    lines = [
        ...
        f"Loss streak warning threshold: **{ls_warning}** (diagnostic only).", "",
    ...
    if len(stable) == 0:
        status = "ETH_R3_H05_TEST_DEGRADATION_FAIL"
        ...
        status = "ETH_R3_H05_LOCAL_NEIGHBORHOOD_FAIL"
        OUT_STATUS.write_text(status + "\n")
        if OUT_LOCK.exists(): OUT_LOCK.unlink()
        ...
        stable = stable.sort_values(
            ["manhattan_distance", "exp_retention", "wr_change_pp", "pf", "dd", "ls", "hold_min", "lookback_min"],
        ...
        pd.DataFrame([{
            ...
            "local_viable_cells": int(len(viable)), "local_stable_cells": int(len(stable)),
            "test_risk_clustering_warning": bool(rep.risk_clustering_warning), "validation_2024_opened": False,
        }]).to_csv(OUT_LOCK, index=False)
        status = "ETH_R3_H05_STABLE_EDGE_FROZEN"
        OUT_STATUS.write_text(status + "\n")
        lines += [
            ...
            f"Risk clustering warning: {'YES' if bool(rep.risk_clustering_warning) else 'NO'}.",
