...
def build_case(x5: pd.DataFrame, anchor: pd.Timestamp, clock_min: int):
    ...
    # B27Q-equivalent first High visit K1 with OPP0 purity.
    hi_touching = False
    lo_touching = False
    hi_visits = 0
    lo_visits = 0
    ...
    # B27W-equivalent causal leave from the contiguous K1 High-touch episode.
    leave_k = None
    ...
    # Terminal search after leave: first H2 or strict opposite close break.
    terminal_k = len(exe)
    ...
    # First F85 fill strictly after causal leave and strictly before terminal/H2 bar.
    touch_k = None
    ...
    # B27AA causality around H2: next open may equal H2 bar start, but cannot open at/above H.
    if entry_px >= H:
    ...
    # Exact B27AA fixed E20 + F35 completed-close invalidation economics.
...
def historical_replication_supported(summary: pd.DataFrame, clock_min: int) -> bool:
    need = {
        'external': (15, 0.65, 1.20),
        'reference_validation': (10, 0.65, 1.20),
    }
...
def main():
    ...
    # Hard parity must pass before any leaderboard/output interpretation.
    ...
    for clock_min in CLOCKS_MIN:
        ...
        for part in PART_ORDER:
            ...
            sums.append({
                'clock_min': clock_min,
                'reference_start_utc': rs,
                'execution_start_utc': es,
                'execution_end_utc': ee,
                'partition': part,
                **summarize(g),
            })
    ...
    for r in leader.head(16).itertuples(index=False):
        lines.append(
            f'| {r.reference_start_utc} | {r.execution_start_utc} | {r.execution_end_utc} | {r.trades} | {pct(r.wr)} | {num(r.pf)} | ${num(r.expectancy)} | ${num(r.total_net)} | {r.k1_opp0} | {r.f85_touches} | {"YES" if r.dev_eligible else "NO"} |'
        )
