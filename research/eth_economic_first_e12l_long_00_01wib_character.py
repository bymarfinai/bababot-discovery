...
PFX = "ETH_ECONOMIC_FIRST_E12L_LONG_00_01WIB_CHARACTER"
...
def main():
    ...
    lines = [
        "# ETH Economic-First E12L — 00:00–01:00 WIB LONG Character Discovery Result", "",
        f"Raw ETHUSDT 5m coverage: **{coverage:.4%}**.",
        "Direction: **LONG only**.",
        "Time habitat: **00:00–01:00 WIB only** (17:00, 17:15, 17:30, 17:45 UTC anchors).",
        "Scientific grammar and gates: **identical to preregistered E12A–E12K; only the hour changed**.",
    ...
    if len(C) == 0:
        status = "ETH_ECONOMIC_FIRST_E12L_NO_LONG_CHARACTER"
        OUT_STATUS.write_text(status + "\n")
        lines += ["", f"**Status: {status}**", "", "No 00:00–01:00 WIB LONG character passed the preregistered anchor-stability + pooled-economics + all-era gates.", "No gate relaxation and no OOS exposure.", "", "Research/shadow only."]
    ...
    status = "ETH_ECONOMIC_FIRST_E12L_LONG_CHARACTER_FOUND"
    OUT_STATUS.write_text(status + "\n")
