...
CELLS_PATH = ROOT / "BNB_R4B_STAGEA_2022_FROZEN_COMPONENT_CELLS.csv"
COMP_PATH = ROOT / "BNB_R4B_STAGEA_2022_FROZEN_COMPONENTS.csv"
OUT_CELLS = ROOT / "BNB_R4B_STAGEB_2023_CELL_RESULTS.csv"
OUT_VERDICTS = ROOT / "BNB_R4B_STAGEB_2023_COMPONENT_VERDICTS.csv"
OUT_SURVIVORS = ROOT / "BNB_R4B_STAGEB_2023_STABLE_PLATEAUS.csv"
OUT_RESULT = ROOT / "BNB_R4B_STAGEB_2023_COMPONENT_SCREEN.md"
OUT_STATUS = ROOT / "BNB_R4B_STAGEB_2023_Status.txt"
...
def main():
    ...
    if cells.empty or comps.empty:
        raise AssertionError("Stage A frozen components are empty")
    if len(comps) != 72:
        raise AssertionError(f"expected 72 frozen components, got {len(comps)}")
    ...
    if idx.max() < pd.Timestamp("2023-12-31 23:55:00", tz="UTC"):
        raise RuntimeError(f"2023 dataset incomplete; max timestamp is {idx.max()}")
    # Prior history is retained for causal warm-up. 2024+ is excluded from feature construction.
    x = x5[idx < END].copy()

    out = []
    for hour in sorted(cells.hour_wib.astype(int).unique()):
        H = cells[cells.hour_wib == hour].copy()
        clocks = stageA.hour_clocks(hour)
        engine.CLOCKS = clocks
        engine.LOOKBACKS = tuple(sorted(H.lookback_min.astype(int).unique()))
        engine.HOLDS = tuple(sorted(H.hold_min.astype(int).unique()))
        cache = engine.prep(x)
        ...
        for c in H.itertuples(index=False):
            ...
            s = stats(T.net.to_numpy(float), T.gross.to_numpy(float))
    ...
    if len(R) != len(cells):
        raise AssertionError(f"frozen cell replay mismatch: {len(R)} vs {len(cells)}")
    ...
    for (hour, rank), G in R.groupby(["hour_wib", "component_rank"], sort=True):
        ...
        row = {
            ...
            "dev_component_wr_min": float(meta.component_wr_min),
            "dev_component_wr_mean": float(meta.component_wr_mean),
            "dev_component_exp_floor": float(meta.component_exp_floor),
            "dev_component_exp_mean": float(meta.component_exp_mean),
            "test_viable_cells": v,
            "test_stable_cells": s,
            ...
            "test_max_ls": int(G.test_ls.max()),
            "risk_warning_cells": int(G.risk_clustering_warning.sum()),
            "verdict": verdict,
        }
        verdicts.append(row)
    ...
    survivor_hours = sorted(S.hour_wib.astype(int).unique().tolist()) if not S.empty else []
    partial = V[V.verdict == "PARTIAL_PLATEAU_PERSISTENCE"]
    narrow = V[V.verdict == "NARROW_STABLE_POINT"]

    lines = [
        "# BNB R4b — Stage B 2023 Connected-Component Screen", "",
        ...
        f"Frozen components tested: **{len(V)}**; frozen cells replayed: **{len(R)}**.",
        f"Stable plateaus: **{len(S)}** across **{len(survivor_hours)}** hours.",
        ...
        "| Hour | Rank | Character | Size | 2023 viable | Stable | Viable % | Median WR | Median Exp | Median PF | Max DD | Max LS | Verdict |",
        "|---:|---:|---|---:|---:|---:|---:|---:|---:|---:|---:|---:|---|",
    ]
    for r in V.itertuples(index=False):
        lines.append(
            ...
            f"${float(r.test_max_dd):.2f} | {int(r.test_max_ls)} | {r.verdict} |"
        )

    if not S.empty:
        lines += ["", "## Stable plateaus frozen for 2024 persistence test", "",
    ...
    lines += ["", "No component or cell may be modified after observing these 2023 results.",
              "2024-2026 remained closed during this run.", "", "Research/shadow only."]
    OUT_RESULT.write_text("\n".join(lines) + "\n")
    OUT_STATUS.write_text("BNB_R4B_STAGEB_COMPLETE\n")
