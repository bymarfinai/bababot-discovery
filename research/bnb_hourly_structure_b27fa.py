...
PFX = 'BNB_HOURLY_STRUCTURE_B27FA'
...
def bounds_for_local_day(day):
    ref_start_local = datetime.combine(day, time(0, 0), tzinfo=WIB)
...
def build_sessions(x5: pd.DataFrame) -> pd.DataFrame:
    ...
    for d in pd.date_range(local_first, local_last, freq='D'):
        ...
        # Require the complete 8-hour geometry to be fully inside development.
        ...
        if len(ref) != 48 or len(exe) != 48:
            raise AssertionError(
                f'incomplete B27FA session {day}: ref={len(ref)}/48 exe={len(exe)}/48'
            )
    ...
    d = pd.DataFrame(rows)
    if d.empty:
        raise AssertionError('no B27FA sessions')
    return d
...
def main():
    ...
    if not prereg.exists():
        raise AssertionError('B27FA preregistration missing')
    ...
    OUT_MD.write_text('\n'.join(lines) + '\n', encoding='utf-8')
    OUT_STATUS.write_text('B27FA_BNB_HOUR00_STRUCTURE_COMPLETE\n', encoding='utf-8')
    print('\n'.join(lines))
